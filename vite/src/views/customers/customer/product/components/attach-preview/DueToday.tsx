import { AttachBranch, getAmountForQuantity, type Price } from "@autumn/shared";
...
export const DueToday = () => {
	...
	const getPrepaidPrice = ({ option }: { option: any }) => {
		...
		if (option.tiers) {
			const _start = option.tiers[0].amount;
			const _end = option.tiers[option.tiers.length - 1].amount;
			return "x ";
			// return `${formatAmount({
			//   amount: start,
			//   defaultCurrency: currency,
								...
								<Input
									...
									onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
										...
										newOptions[index].quantity =
											parseInt(e.target.value, 10) * billing_units;
