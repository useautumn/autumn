import { X } from "lucide-react";

import { Button } from "@/components/ui/button";
import { Entity } from "@autumn/shared";
import { useLocation, useNavigate } from "react-router";

export const EntityHeader = ({ entity }: { entity?: Entity }) => {
  const location = useLocation();
  const navigate = useNavigate();

  if (!entity) {
    return null;
  }

  return (
    <div className="flex items-center gap-2 bg-muted/50 text-muted-foreground text-sm px-3 py-1 rounded-md pr-1 h-6">
      <span className="">Entity:</span>
      <span className="font-mono">{entity.name}</span>
      <Button
        variant="ghost"
        size="icon"
        className="w-5 h-5 hover:bg-primary/5 rounded-md"
        onClick={() => {
          const params = new URLSearchParams(location.search);
          params.delete("entity_id");
          navigate(`${location.pathname}?${params.toString()}`);
        }}
      >
        <X size={14} />
      </Button>
    </div>
  );
};
