...
export const PricingConfig = ({
	...
	const [_name, _setName] = useState(priceConfig?.name || "");
	const [_usageTiers, _setUsageTiers] = useState<any[]>([]);
	const [fixedConfig, _setFixedConfig] = useState(
		priceConfig && priceConfig.type === PriceType.Fixed
			? priceConfig
			: defaultFixedConfig,
	);
	const [_usageConfig, _setUsageConfig]: any = useState(
		priceConfig?.config && priceConfig.config.type === PriceType.Usage
			? priceConfig.config
			: defaultUsageConfig,
	);

	const [_originalPrice, _] = useState(priceConfig);
...
export const validateConfig = (price: any, _prices: any) => {
