import type { LinkLicenseParams, PlanLicense } from "@autumn/shared";
...
export const usePlanLicensesQuery = (parentPlanId?: string) => {
	...
	const { data, isLoading, error, refetch } = useQuery<{ list: PlanLicense[] }>(
		{
			...
			queryFn: async () => {
				const { data } = await axiosInstance.post("/v1/licenses.list_links", {
	...
	const linkLicense = useMutation({
		mutationFn: async (params: LinkLicenseParams) => {
			const { data } = await axiosInstance.post("/v1/licenses.link", params);
			return data;
		},
