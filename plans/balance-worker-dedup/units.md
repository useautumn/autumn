...
| 8 | **Async and batch track use it.** `async: true` on the worker path: the API claims the key, `queue.track`, 202; a failed append releases it. Batch: one command per item with `${ctx.id}-${index}` as its request id, one `queue.track`, and the API claims nothing: a keyed item carries `idempotency: { key, ttlMs }` on its commands and the consumer claims it, as the SQS consumer does today. | `async-track-worker` | server unit: async claims and keeps, 503 releases; batch queues once with the key on the commands. worker unit: `consumeTrack` claims under the item's request id, skips another item's key, releases on refusal, keeps on transient |
...
| 8 | done, uncommitted. `packages/dynamodb` (`@autumn/dynamodb`, shaped like `postgres`): client, table constants, storage key, the claim/release repo with owner semantics (`claimed` / `resumed` / `duplicate` / `unavailable`), `createIdempotencyKeyStore`, and `withIdempotencyKey` (claim → run → release on a retryable failure; `onDuplicate` decides 409 or skip). The server's DynamoDB modules re-export it. Engine: `TrackCommand.idempotency?: { key, ttlMs }`. Worker: `dynamo` beside `postgres` in resources, `idempotencyKeys` bound from it and threaded to `ConsumeContext`; `consumeTrack` runs the processor under `withIdempotencyKey` with `owner: command.requestId`, so an item's fan-out commands and redeliveries resume one claim while another item's key is a duplicate; `consume/trackSettlement.ts` names what a failure comes to. Server: `runBalanceWorkerAsyncTrack`, `runBalanceWorkerBatchTrack`, both handlers branch on the rollout. Not done: the server's own `withIdempotencyKey` still wraps its action modules rather than the package hook; worker env needs `DYNAMODB_ENDPOINT` locally and the task role needs table access. |
