...
# MCP configs generated by a prior `bun ai sync` are copied from the canonical
# checkout before the first agent starts (see .worktreeinclude). Refresh them
# here for later sessions, along with skills and rules; dw's own checkout has no
# credential.
...
# Cloud sessions may not have a personal shell environment, so resolve Claude's
# `${EXECUTOR_API_KEY}` placeholder after sync when the key is available from
# Infisical. .mcp.json is gitignored, so it cannot leak into a commit.
