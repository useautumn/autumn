...
- **Cloudflare** — `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` in `~/.autumn-agent/cloudflare.env` (or `cloudflared tunnel login`) so dw can create `autumn-wtN-<hash>[-service].autumnworktree.com`
