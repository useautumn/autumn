...
if token="$(bash "$ROOT/scripts/setup/infisical-machine-login.sh")"; then
	...
	bash "$ROOT/scripts/setup/export-dev-secrets.sh"
...
cat > "$env_sh" <<ENVSH
# Generated by cursor-cloud-start.sh — sourced from bashrc / BASH_ENV / zshenv.
export DW_HEADLESS=1
export PATH="${ROOT}/node_modules/.bin:/usr/local/bin:\$HOME/.bun/bin:\$PATH"
if [ -z "\${INFISICAL_TOKEN:-}" ] && [ -n "\${INFISICAL_CLIENT_ID:-}\${INFISICAL_UNIVERSAL_AUTH_CLIENT_ID:-}" ]; then
  INFISICAL_TOKEN="\$("${ROOT}/scripts/setup/infisical-machine-login.sh" 2>/dev/null || true)"
