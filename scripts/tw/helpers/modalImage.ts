...
export const buildBaseImage = (
	...
	const playwrightVersion = resolveBakedPlaywrightVersion();
	return (
		modal.images
			...
			//    test files use the LOCAL Playwright path). The version is pinned from the
			//    HOST lockfile (resolveBakedPlaywrightVersion) — it must equal what the test
			//    process's `server/node_modules/playwright-core` resolves, since
			//    `chromium.executablePath()` embeds that version's browser revision.
			//    `--with-deps` apt-installs the OS libs (refresh the index first — earlier
			//    layers cleared it). Uses `bun x` (not `bunx`) — only `bun` is symlinked onto
			//    /usr/local/bin — and puts /root/.bun/bin on PATH. The final probe fails the
			//    build loudly rather than shipping an image with no launchable browser.
			.dockerfileCommands([
				`# chromium bake — playwright ${playwrightVersion}\n` +
