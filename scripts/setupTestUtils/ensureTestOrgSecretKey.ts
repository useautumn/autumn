...
import { apiKeyRepo } from "@server/internal/dev/repos/index.js";
...
export async function ensureTestOrgSecretKey({
	...
	db: DrizzleCli;
}): Promise<{ key: string; alreadyExists: boolean; generated: boolean }> {
	...
	if (!hardcodedKey) {
		const existingKeys = await apiKeyRepo.listByOrg({
			db,
			orgId: TEST_ORG_CONFIG.id,
			env: AppEnv.Sandbox,
		});
		for (const existingKey of existingKeys) {
			const meta = existingKey.meta as Record<string, unknown> | null;
			if (
				existingKey.name !== "Unit Test Key" ||
		...
		return { key, alreadyExists: false, generated: true };
	}
