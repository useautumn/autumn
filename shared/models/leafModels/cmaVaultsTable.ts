...
export const cmaVaults = leafSchema.table(
	"cma_vaults",
	{
		...
		// Per-user for web chat; NULL for installation-scoped (Slack) vaults.
	...
	(table) => [
		// NULLS NOT DISTINCT so installation-scoped rows (NULL user_id) still
		// dedupe + upsert-on-conflict instead of inserting duplicates.
		unique("cma_vaults_installation_org_env_user_key")
			.on(table.chat_installation_id, table.org_id, table.env, table.user_id)
			.nullsNotDistinct(),
	],
);
