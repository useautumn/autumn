/** biome-ignore-all lint/style/noNonNullAssertion: copied from server item mapper; kept structurally close for parity */
...
const entitlementId = () => generateKsuid({ prefix: "ent_" });
...
export const toFeature = ({
	...
	let ent: Entitlement = {
		id: item.entitlement_id || curEnt?.id || entitlementId(),
		org_id: orgId,
		created_at: item.created_at || Date.now(),
		is_custom: isCustom,
		internal_product_id: internalProductId || null,
