// forked from design-sync lib/dts.mjs — @autumn/ui re-exports its whole API through
// the `@autumn/ui/*` tsconfig path alias, which the bundled projectFor can't resolve
// (it hardcodes compilerOptions and never reads cfg.tsconfig), yielding 7 of 214 exports.
...
// Nearest tsconfig at/above pkgDir, so `paths` aliases resolve in the checker.
function tsconfigOptionsFor(pkgDir) {
  for (let d = pkgDir; ; d = dirname(d)) {
    ...
    if (existsSync(p)) {
      ...
      return { baseUrl: join(d, opts.baseUrl || '.'), paths: opts.paths };
    }
