...
describe("parseMeteringRecord without the snapshot", () => {
	test("drops `after` and still validates the rest of the record", () => {
		const mutation = {
			...createMutation(),
			...
			after: {
				state: {
					schemaVersion: 1,
					identity: createMutation().identity,
					revision: 1,
					customer: {
						internal_id: "c",
						id: "c",
		...
		const serialized = serializeMeteringRecord({ record: mutation });
		const read = parseMeteringRecord({ ...serialized, snapshot: "skip" });
		expect("after" in read).toBe(false);
		expect(read.source).toEqual({ commandOffset: "41" });
		// The full read still carries the snapshot.
		expect(parseMeteringRecord(serialized).after).toBeDefined();
		// A bad offset is refused whether or not the snapshot is read.
		const bad = { ...mutation, source: { commandOffset: "-1" } };
		expect(() =>
			parseMeteringRecord({
				...serializeTopicRecord({ key: "k", record: bad }),
				snapshot: "skip",
			}),
		).toThrow(InvalidRecordError);
	});
