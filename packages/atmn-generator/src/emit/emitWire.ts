 ...
 * empty".
 */
/** A predicate a `rejects` lint rule names, imported from a copied runtime module. */
export type LintCheckImport = {
	readonly name: string;
	/** Module under the generated folder, e.g. `./isLocalWebhookUrl.js`. */
...
export const emitWireModule = ({
	...
	checks: readonly LintCheckImport[];
}): string => {
	const entries = Object.entries(collections);
	const singletonEntries = Object.entries(singletons);
	const listEntries = Object.entries(syncedLists);
	const imports = [
		...[...entries, ...singletonEntries, ...listEntries].map(
			([name, meta]) => `import type { ${meta.typeName} } from "./${name}.js";`,
		),
		...checks.map(({ name, from }) => `import { ${name} } from "${from}";`),
	].join("\n");
	const listMembers = listEntries
		...
		.join("\n");
	const listStated = listEntries
		.map(
			([name]) =>
				`\t...(config.${name} !== undefined ? { ${name}: config.${name} } : {}),`,
		)
		.join("\n");
	const listKeys = JSON.stringify(listEntries.map(([name]) => name));
	const listBodies = listEntries
		.map(
			([name]) =>
				`\t\t${name}: document.${name} as WireDocument[] | undefined,`,
		)
		.join("\n");
	const checkNames = checks.map(({ name }) => name).join(", ");
...
const LIST_KEYS: readonly string[] = ${listKeys};
