...
const removeMemberEdits = ({ member }: { member: SgNode }): Edit[] => {
		...
		if (typeof segment === "number") {
			...
			if (member === undefined) {
				...
				for (const key of remaining.reverse())
					inserted = `{ ${JSON.stringify(key)}: ${inserted} }`;
				const pair = `${JSON.stringify(segment)}: ${inserted}`;
...
export const patchFixturePaths = ({
	...
	for (const assignment of assignments) {
		...
		const edits =
			object === null
				? "ambiguous-fixture"
				: pathEdits({ source, object, builders, ...assignment });
