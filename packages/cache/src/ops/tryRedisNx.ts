...
/** Routes a SET … NX to its three outcomes: "OK" → onSuccess, null (key exists) → onKeyAlreadyExists, Redis down → onRedisUnavailable. */
export const tryRedisNx = async <TUnavailable, TSuccess, TExists>({
	...
	operation: (redis: Redis) => Promise<"OK" | null>;
	source: string;
	redisInstance: Redis;
