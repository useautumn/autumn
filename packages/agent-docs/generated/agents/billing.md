...
- When the requested change can be faithfully represented, resolve every ambiguity decisively, state the assumption in your preview line, and build — the preview IS how you confirm. A requested term that does not resolve is missing: stop rather than omit, substitute, or invent it.
