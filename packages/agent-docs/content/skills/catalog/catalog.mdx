...
Triggered whenever the customer pays per unit of some entity — seats, workspaces, projects, sites, members: "each X is $10/month", "comes with 3 X". When you see one, always ask: **what does one X come with?** Never skip this because the user didn't say "seat". <pointer file="../concepts/references/licenses.md" when="modeling a license — the concept: child plan, license link, customize" />
...
<part file="references/fork-licenses.md" when="deciding between a per-unit item, licenses, and entity-attached plans for a countable paid unit" />
...
<part file="references/fork-variants.md" when="deciding whether volume buckets/tiers are variants of the plan or one prepaid volume-tiered item" />
...
<part file="references/fork-addon.md" when="modeling packs, top-ups, or any purchase bought on top of a plan — add-on vs plan item, and what level it sits at" />
...
<part file="references/fork-pooled.md" when="deciding where balances and purchases live — pooled grants, customer-level packs" />
...
<part file="references/cases.md" when="checking the derived structure against known-good shapes" />
...
<reference url="/documentation/modelling-pricing/usage-based-pricing" when="charging for usage beyond an included allowance" />

<reference url="/documentation/modelling-pricing/prepaid-pricing" when="selling a quantity bought upfront (seats, credit packs, buckets)" />

<reference url="/documentation/modelling-pricing/volume-based-tiers" when="pricing that changes by volume tier" />

<reference url="/documentation/modelling-pricing/per-unit-pricing" when="modeling $X per unit — always with a base fee" />

<reference url="/documentation/modelling-pricing/recurring" when="recurring plans; billing interval vs reset interval" />

<reference url="/documentation/modelling-pricing/one-off-purchases" when="one-off purchases or top-up items" />

<reference url="/documentation/modelling-pricing/auto-top-ups" when="auto-recharge behavior and the one-off prepaid item it requires" />

<reference url="/documentation/modelling-pricing/rollovers" when="unused allowance carrying over" />

<reference url="/documentation/modelling-pricing/trials" when="trial details — durations, card behavior, what happens on expiry" />

<reference url="/documentation/modelling-pricing/entity-plans" when="entity-scoped plans and licenses — attach flows, provisioning" />

<reference url="/documentation/modelling-pricing/credit-systems" when="credit schemas, action mappings, monetary credits" />

<reference url="/documentation/modelling-pricing/free-plans" when="default plans and auto-enable rules" />

<reference url="/documentation/modelling-pricing/add-ons" when="add-on plans and balance stacking" />

<reference url="/documentation/modelling-pricing/plan-variants" when="creating variants — what they can and cannot change" />
...
<part file="references/atmn.md" when="using atmn, autumn.config.ts, or headless push flows" />

<part file="references/catalog-update.md" when="changing an existing catalog: previewing, versioning, migrations, variant propagation" />
