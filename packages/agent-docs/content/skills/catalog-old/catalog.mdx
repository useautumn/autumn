...
2. Preview it: `atmn` for `autumn.config.ts` projects (commands in `references/atmn.md`), or `catalog.preview_update` for MCP/API flows.
...
<part file="references/catalog-update.md" when="changing an existing catalog, previewing catalog changes, or deciding between versioning, in-place updates, migrations, and variant propagation" />

<part file="references/atmn.md" inline="true" />

<part file="../concepts/references/plan.md" when="modeling annual/monthly variants, A/B plan variants, or volume-ladder variants" />
