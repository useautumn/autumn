...
<part file="actions.md" when="choosing which billing action to run (attach, updateSubscription, createSchedule)" />
...
<part file="customize.md" when="applying customer-specific plan terms or price overrides" />

<part file="timing-schedules.md" when="resolving when a billing change takes effect or building schedule phases" />

<part file="billing-behavior.md" when="resolving invoice, checkout, and proration behavior" />
