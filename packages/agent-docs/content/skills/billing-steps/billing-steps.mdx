...
Then decide how it is paid. Follow the user's instructions or the org rules. If neither says, first check if there is a payment method (from the `getCustomer` call), and follow these defaults:

- If payment method exists, just attach directly (without invoice mode) to charge it.
- Otherwise, use Invoice mode: `invoice_mode.enabled: true`, `invoice_mode.finalize: true`,
- Use`enable_plan_immediately: true`
- Timing: always set `plan_schedule: "immediate"`. DO NOT omit this, as Autumn will default to schedule a lower priced plan.
- Proration: `proration_behavior: "none"` — but omit it entirely if the customer has no existing subscription.
- Checkout URL instead: omit `invoice_mode`, set `redirect_mode: "always"`, with long_lived_checkout: true
