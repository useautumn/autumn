...
async function main() {
	...
	console.log(
		chalk.gray("Dry-run performance testing with realistic workloads"),
	);
	console.log(
		chalk.gray(
			`${new Date().toLocaleString()} | Node ${process.version} | ${process.platform}\n`,
		),
	);
	...
	const suitesToRun = suiteFilter
		? BENCHMARK_SUITES.filter(
				(suite) =>
					suite.name.toLowerCase().includes(suiteFilter.toLowerCase()) ||
					suite.name
						.toLowerCase()
						.replace(/\s+/g, "")
						.includes(suiteFilter.toLowerCase()),
			)
		: BENCHMARK_SUITES.filter((suite) => suite.enabled);

	if (suitesToRun.length === 0) {
		console.log(
			chalk.red(`❌ No benchmark suites found matching: ${suiteFilter}`),
		);
		console.log(chalk.yellow("\nAvailable suites:"));
		BENCHMARK_SUITES.forEach((suite) => {
			console.log(
				chalk.yellow(`  • ${suite.name.toLowerCase().replace(/\s+/g, "")}`),
			);
		});
		process.exit(1);
	}
	...
	allResults.forEach((suiteResult) => {
		...
		fastOperations += suiteResult.results.filter(
			(r: any) => r.averageTime < 5,
		).length;

		const avgTime =
			suiteResult.results.reduce(
				(sum: number, r: any) => sum + r.averageTime,
				0,
			) / suiteResult.results.length;
		const timeColor =
			avgTime < 5 ? chalk.green : avgTime < 20 ? chalk.yellow : chalk.red;

		console.log(
			`${chalk.cyan(suiteResult.suite.padEnd(25))} ${timeColor(`${avgTime.toFixed(1)}ms avg`)} ${chalk.gray(`(${suiteResult.results.length} tests)`)}`,
		);
	});
