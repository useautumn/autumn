...
local function check_usage_window_limits(params)
  ...
  for _, limit in ipairs(limits) do
    ...
    if consumed > 0 then
      ...
      if current_usage + consumed > safe_number(limit.limit) then
        return limit.feature_id
      end
    end
  end
...
-- current window key) and marks the anchor dirty so apply_pending_writes
-- persists it, even when the anchor's own balance did not change.
  ...
  for _, limit in ipairs(limits) do
    ...
    if not is_nil(windows) then
      ...
      if consumed > 0 then
        ...
        -- Drop stale counters from windows that have already closed so the
        -- embedded map does not grow unbounded (lifetime never closes).
