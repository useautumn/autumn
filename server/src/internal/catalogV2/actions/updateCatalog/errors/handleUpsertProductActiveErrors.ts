...
export const handleUpsertProductActiveErrors = ({
	...
	for (const planParams of params.plans ?? []) {
		...
		throw new RecaseError({
			message: `Cannot set active to false on plan "${planParams.plan_id}": no version of it is active in this update. Exactly one version of each plan must be active: true.`,
			code: ErrCode.InvalidRequest,
			statusCode: 400,
		});
	}
