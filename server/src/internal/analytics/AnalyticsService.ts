...
order by dr.period;
	...
	static async getBillingCycleStartDate(
		...
		const customerProductsFiltered = AnalyticsService.filterActiveProducts(customer.customer_products);
		console.log("customerProductsFiltered", customerProductsFiltered);

		if (customerProductsFiltered.length === 0 || !customerProductsFiltered) {
			return {};
		}

		const fullProducts = await AnalyticsService.getFullProducts(
			customerProductsFiltered,
			db,
			env,
			customer.org_id
		);

		const areAllProductsFree = AnalyticsService.checkIfAllProductsAreFree(fullProducts);
		const { startDates, endDates } = areAllProductsFree
			? AnalyticsService.getDateRangesFromEntitlements(customer.customer_products)
			: AnalyticsService.getDateRangesFromSubscriptions(customerProductsFiltered, subscriptions);
		...
		return AnalyticsService.calculateBillingCycleResult(startDates, endDates, intervalType);
	}
	...
	private static filterActiveProducts(customerProducts?: FullCusProduct[]): FullCusProduct[] {
		return customerProducts?.filter((product: FullCusProduct) => {
			const isActive =
	...
	private static async getFullProducts(
		...
		const fullProducts = await Promise.all(
			customerProductsFiltered.map(async (product: FullCusProduct) => {
				return await ProductService.getFull({
					db,
					idOrInternalId: product.product.internal_id,
					env,
					orgId,
				});
			})
	...
	private static checkIfAllProductsAreFree(fullProducts: FullProduct[]): boolean {
	...
	private static getDateRangesFromEntitlements(
		...
		console.log("customer_entitlements", JSON.stringify(customerProducts?.map(x => x.customer_entitlements), null, 4));
		...
		customerProducts.forEach((product: FullCusProduct) => {
			if (!product.customer_entitlements || product.customer_entitlements.length < 1) {
				return;
			}

			product.customer_entitlements?.forEach((entitlement: FullCustomerEntitlement) => {
				if (entitlement.next_reset_at) {
					endDates.push(AnalyticsService.formatDateToString(new Date(entitlement.next_reset_at)));
				}
	...
	private static calculateStartDateFromInterval(
		...
		switch (interval) {
			...
			case EntInterval.Minute:
				return AnalyticsService.formatDateToString(new Date(nextResetAt! - 60 * 1000));
			case EntInterval.Hour:
				return AnalyticsService.formatDateToString(new Date(nextResetAt! - 60 * 60 * 1000));
			case EntInterval.Day:
				return AnalyticsService.formatDateToString(new Date(nextResetAt! - 24 * 60 * 60 * 1000));
			case EntInterval.Week:
				return AnalyticsService.formatDateToString(new Date(nextResetAt! - 7 * 24 * 60 * 60 * 1000));
		...
		customerProductsFiltered.forEach((product: FullCusProduct) => {
			product.subscription_ids?.forEach((subscriptionId: string) => {
				const subscription = subscriptions.find(
					(subscription: Subscription) => subscription.stripe_id === subscriptionId
				);
