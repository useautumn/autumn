...
order by dr.period;
  ...
  WHERE e.org_id = {organizationId:String}
    AND e.env = {env:String}
    AND e.timestamp >= toDateTime({startDate:String})
    AND e.timestamp < toDateTime({endDate:String})
    ${aggregateAll ? "" : "AND e.customer_id = {customerId:String}"}
