...
WHERE 
  NOT EXISTS ( 
	...
	static async getTotalEvents({ req, eventName }: { req: ExtendedRequest, eventName?: string }) {
		const { clickhouseClient, org, env, db } = req;
  ...
  AND env = {env:String};`;
	...
	static async getTimeseriesEvents({
		...
		const expressionsResult = await clickhouseClient.query({
			query: "SELECT generateEventCountExpressions({event_names:Array(String)}) as expressions",
			query_params: {
