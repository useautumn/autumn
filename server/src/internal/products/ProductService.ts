...
export class ProductService {
	...
	static async getDeletionText({
		...
		let res = await db
			.select({
				customerName: sql<string>`CASE WHEN ROW_NUMBER() OVER (ORDER BY ${customerProducts.created_at}) = 1 THEN ${customers.name ?? "name not found"} ELSE NULL END`,
