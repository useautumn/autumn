...
export const bucketOpsBySubscription = ({
	...
	for (const op of operations.update_plans ?? []) {
		const matched = cusProducts.filter((cusProduct) =>
			planFilterMatchesCustomerProduct({ filter: op.target, cusProduct }),
		);
	...
	for (const [subId, matches] of groups) {
		if (subId !== null) buckets.push({ stripeSubscriptionId: subId, matches });
	}
	const nullMatches = groups.get(null);
	if (nullMatches?.length)
		buckets.push({ stripeSubscriptionId: null, matches: nullMatches });
