...
cusRouter.get("/:customer_id/events", async (req: any, res: any) => {
	try {
		...
		const _period = req.query.period || "all";
...
cusRouter.get("/:customer_id/data", async (req: any, res: any) => {
	try {
		...
		const fullCustomer = customer as any;
...
cusRouter.get("/:customer_id/referrals", async (req: any, res: any) => {
	try {
		...
		const [referred, redeemed] = await Promise.all([
		...
		try {
			...
			if (notNullish(customer_product_id)) {
				...
				cusProduct = cusProducts.find(
					(p: any) =>
						...
						p.product.version === parseInt(version, 10) &&
						(entity ? p.internal_entity_id === entity.internal_id : true),
				);
			} else {
				cusProduct = cusProducts.find(
			...
			const product = cusProduct
				...
				: await ProductService.getFull({
						...
						version:
							version && Number.isInteger(parseInt(version, 10))
								? parseInt(version, 10)
								: undefined,
					});
			...
			const numVersions = await ProductService.getProductVersionCount({
				db,
				orgId: org.id,
				env,
