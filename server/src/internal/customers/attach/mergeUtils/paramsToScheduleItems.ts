...
export const removeCusProductFromScheduleItems = ({
  ...
  for (const price of prices) {
    ...
    if (isArrearPrice({ price })) {
      ...
      //   price: (existingScheduleItem.price as Stripe.Price).id,
      //   quantity: 0,
      // } as any);
...
export const paramsToScheduleItems = async ({
  ...
  let newScheduleItems = mergeNewScheduleItems({
    itemSet,
    curScheduleItems,
  });
  ...
  for (const cusProduct of cusProductsToRemove) {
    ...
    for (const price of prices) {
      ...
      if (isArrearPrice({ price })) {
        ...
        newScheduleItems.push({
          price: existingScheduleItem.price as string,
          quantity: 0,
        } as any);
      ...
      if (existingItemIndex !== -1) {
        ...
        if (newQuantity <= 0) {
          removePriceIds.push(
            (newScheduleItems[existingItemIndex].price as Stripe.Price).id
          );
        }
  ...
  // newScheduleItems = newScheduleItems
  //   .filter((si) => !removePriceIds.includes((si.price as Stripe.Price).id))
  //   .map((si) => ({
