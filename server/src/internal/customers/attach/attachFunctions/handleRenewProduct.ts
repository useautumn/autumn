...
export const handleRenewProduct = async ({
  ...
  // Are there any other products canceled (on this schedule ID)
  const otherCanceled = cusProducts.some(
    (cp) =>
      cp.scheduled_ids?.includes(schedule.id) &&
      cp.canceled &&
      cp.id !== curCusProduct?.id
  );

  if (!otherCanceled) {
    console.log("NO OTHER CANCELED PRODUCTS, CANCELING SCHEDULE");
