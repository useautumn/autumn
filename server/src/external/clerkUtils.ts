...
export const createClerkOrg = async (name: string, slug: string) => {
	...
	try {
		const org = await clerkCli.organizations.createOrganization({
			name,
			slug,
		});
		return org;
...
export const getStripeKey = async (orgId: string, _env: AppEnv) => {
	const orgRes = await getOrgById(orgId);
	const meta: any = orgRes.privateMetadata;
	...
	const key = meta.stripe?.test_api_key;
