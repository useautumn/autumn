...
export class ClickHouseManager {
	...
	private async initializeClickHouse(): Promise<void> {
		...
		if (!ClickHouseManager.clickhouseAvailable) {
			console.log("0. ClickHouse is not available, please set the CLICKHOUSE_URL, CLICKHOUSE_USERNAME, and CLICKHOUSE_PASSWORD environment variables.");
			return;
		}
	...
	private async ensureQueriesExist() {
        ...
        if (process.env.CLICKHOUSE_SKIP_ENSURES?.toLowerCase() === 'true') {
            console.group();
            console.log('✓ Skipping query ensures - queries assumed to exist already');
            console.groupEnd();
            return;
        }
        
		const queries = [ClickHouseQuery.CREATE_DATE_RANGE_BC_VIEW, ClickHouseQuery.CREATE_DATE_RANGE_VIEW, ClickHouseQuery.CREATE_ORG_EVENTS_VIEW, ClickHouseQuery.CREATE_GENERATE_EVENT_COUNT_EXPRESSIONS_FUNCTION];
	...
	private async executeQuery(
		...
		if (query === ClickHouseQuery.CREATE_GENERATE_EVENT_COUNT_EXPRESSIONS_FUNCTION) {
