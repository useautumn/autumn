...
describe(`${chalk.yellowBright("08_pkey: Testing publishable key")}`, () => {
	...
	const customerId = "pkeyTestCustomer";
	...
	it("should return error if try to downgrade to free", async () => {
		try {
	...
	it("should return correct metered1 amount for pro", async () => {
	...
	it("should return same balance for entitled with bearer and x-publishable-key", async () => {
		const { data } = await bearerPublicAxios.post("/v1/entitled", {
			customer_id: customerId,
			feature_id: features.metered1.id,
		});
	...
	it("should return error when try to send event", async () => {
		try {
