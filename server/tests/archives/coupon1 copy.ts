import { APIVersion, type Customer } from "@autumn/shared";
...
describe(
	...
	() => {
		...
		let _autumn: AutumnInt = new AutumnInt({ version: APIVersion.v1_4 });
		...
		it("CYCLE 1: should set usage to -100 and advance clock by 1 month", async () => {
			const usage = 100;
			const _res = await AutumnCli.usage({
				customerId,
