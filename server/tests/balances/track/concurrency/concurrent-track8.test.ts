...
const testCase = "concurrent-track8";
...
const TOTAL_INCLUDED_USAGE = lifetimeMessagesItem.included_usage ?? 0;
...
describe(`${chalk.yellowBright(`${testCase}: Stress test with 500 concurrent requests per customer for tracking allocated features`)}`, () => {
	...
	test("should have initial balances for all customers", async () => {
		for (const customerId of customerIds) {
			...
			console.log(`  Balance: ${customer.features[TestFeature.Users].balance}`);
			console.log(`  Usage: ${customer.features[TestFeature.Users].usage}`);

			// Total balance should be lifetime only
			expect(customer.features[TestFeature.Users].balance).toBe(
				TOTAL_INCLUDED_USAGE,
			);
			expect(customer.features[TestFeature.Users].usage).toBe(0);
		}
	});
	...
	test("should have correct cached balances for all customers", async () => {
		for (const customerId of customerIds) {
			...
			const actualBalance = new Decimal(
				customer.features[TestFeature.Users].balance ?? 0,
			)
				.toDP(5)
				.toNumber();
			...
			const actualUsage = new Decimal(
				customer.features[TestFeature.Users].usage ?? 0,
			)
				.toDP(5)
				.toNumber();
