...
export const group = "referrals8";

describe(`${chalk.yellowBright(
	"referrals8: Testing referrals (immediate, paid, one-off add-on, referrer only)",
)}`, () => {
	const mainCustomerId = "main-referral-8";
	const redeemer = "referral8-r1";
	...
	it("should create code once", async () => {
		referralCode = await autumn.referrals.createCode({
			customerId: mainCustomerId,
			referralId: referralPrograms.paidAddOnReferrer.id,
		});
