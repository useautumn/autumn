...
class TestRunner {
	...
	constructor({
		...
		if (silentMode) this.silentMode = silentMode;
	}

	async collectTestFiles(paths: string[]): Promise<string[]> {
		...
		try {
			...
			if (proc.stdout) {
				for await (const chunk of proc.stdout) {
					...
					// Only stream output if not in silent mode
					if (!this.silentMode) {
			...
			if (proc.stderr) {
				for await (const chunk of proc.stderr) {
					...
					// Only stream errors if not in silent mode
					if (!this.silentMode) {
			...
			if (proc.exitCode === 0) {
				...
				// In silent mode, immediately output completion for real-time tracking
				if (this.silentMode) {
				...
				// In silent mode, immediately output failure for real-time tracking
				if (this.silentMode) {
					console.log(`✗ ${fileName}`);
				}
			}
	...
	async run(directories: string[]): Promise<void> {
		...
		if (this.testFiles.length === 0) {
			if (!this.silentMode) {
				console.log(
					chalk.yellow("No test files found in specified directories"),
				);
			}
			return;
		}
		...
		// Only setup UI if not in silent mode
		if (!this.silentMode) {
		...
		// Silent mode: results already output as tests complete, no need to output again
	}
...
for (const arg of args) {
	if (arg.startsWith("--max=")) {
		...
		console.log(
			"Usage: bun scripts/testScripts/runTests.ts <dir1> [dir2] [...] [--max=N] [--compact] [--silent]",
		);
		process.exit(1);
	} else {
...
// Run tests
const runner = new TestRunner({ maxParallel, compactMode, silentMode });
