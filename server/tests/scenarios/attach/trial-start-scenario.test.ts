...
test(`${chalk.yellowBright("autumn-checkout: free → pro with trial (card on file) - Trial start")}`, async () => {
	...
	const checkoutUrl = trialResult.url;
	if (!checkoutUrl) {
		console.log("No checkout URL returned");
		return;
	}

	// Parse checkout ID from URL (format: .../checkouts/{checkout_id})
	const checkoutId = checkoutUrl.split("/checkouts/")[1];
	...
	const checkoutData = (await checkoutResponse.json()) as GetCheckoutResponse;
