/**
 * Benchmarks the license item-edit (supersede) batch path end to end against
 * the bench org: seed pools + assignments that ALREADY hold the feature →
 ...
 *
 * The add benchmark measures inserting a feature assignments lack. This one
 * measures repointing a feature they already have, which is the shape a real
 * allowance edit takes (diffPlanV1 emits remove + add sharing a match key).
 *
 * Correctness is asserted after the run: every live assignment must carry
 * exactly one row for the feature, pointing at the minted entitlement, with
 * the allowance delta credited onto the balance it started with.
 */
...
const main = async () => {
	...
	const nextAllowance = (existing.allowance ?? 0) * 2;
	const expectedBalance =
		STARTING_BALANCE + nextAllowance - (existing.allowance ?? 0);
	...
	const migrationId = "bench-mig-license-item-edit";
	await db.execute(
							...
							upsert_licenses: [
								{
									...
									customize: {
										add_items: [
											{
												feature_id: BENCH_LICENSE_FEATURE_ID,
												included: nextAllowance,
												...(existing.interval
													? { reset: { interval: existing.interval } }
													: {}),
											},
										],
										remove_items: [
											{
												feature_id: BENCH_LICENSE_FEATURE_ID,
												...(existing.interval
													? { interval: existing.interval, interval_count: 1 }
													: {}),
											},
										],
									},
	...
	const [counts]: Array<{
		...
		SELECT
			...
			(SELECT count(*) FROM customer_entitlements ce
			 WHERE ce.id LIKE ${`${BENCH_SEAT_ENTITLEMENT_PREFIX}%`}
				AND ce.entitlement_id <> ${existing.id}) AS repointed,
			(SELECT count(*) FROM customer_entitlements ce
			 WHERE ce.id LIKE ${`${BENCH_SEAT_ENTITLEMENT_PREFIX}%`}
				AND ce.entitlement_id = ${existing.id}) AS stale,
			(SELECT count(*) FROM (
				SELECT ce.customer_product_id
				FROM customer_entitlements ce
				WHERE ce.customer_product_id LIKE ${`${BENCH_ASSIGNMENT_PREFIX}%`}
					AND ce.internal_feature_id = ${existing.internal_feature_id}
				GROUP BY ce.customer_product_id HAVING count(*) > 1
			) AS dupes) AS duplicates,
			(SELECT count(*) FROM customer_entitlements ce
			 WHERE ce.id LIKE ${`${BENCH_SEAT_ENTITLEMENT_PREFIX}%`}
				AND ce.balance IS DISTINCT FROM ${expectedBalance}) AS wrong_balance
	`);
	...
	console.log("");
	console.log(`bench: allowance ${existing.allowance} -> ${nextAllowance}`);
	console.log(
		`bench: assignments ${counts.assignments} (expected ${expectedAssignments})`,
	);
	console.log(`bench: repointed ${counts.repointed}, stale ${counts.stale}`);
	console.log(
		`bench: duplicates ${counts.duplicates}, wrong balance ${counts.wrong_balance} (expected ${expectedBalance})`,
	);
	...
	const correct =
		...
		Number(counts.repointed) === expectedAssignments;
