...
test.concurrent(`${chalk.yellowBright("cleanup: oneoff-ended-at-expired")}`, async () => {
	const customerId = "cleanup-oneoff-ended-at-expired";
	...
	await cleanupOneOffCustomerProducts({ ctx, nowMs: endedAt + 1 });
