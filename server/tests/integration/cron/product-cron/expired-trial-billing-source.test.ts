...
test.concurrent(
	`${chalk.yellowBright("product-cron: a priced license keeps its trial parent active")}`,
	async () => {
		...
		expect(
			dbState.products.find(({ id }) => id === trialParent.id)?.status,
...
test.concurrent(
	`${chalk.yellowBright("product-cron: a free license still lets an unbilled trial parent expire")}`,
	async () => {
