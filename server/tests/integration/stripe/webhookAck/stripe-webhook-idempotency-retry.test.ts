...
const waitForRedisReady = async () => {
	...
	while (miscRedis.status !== "ready") {
		if (Date.now() > deadline) {
			throw new Error(`Redis never became ready (status: ${miscRedis.status})`);
		}
...
test.concurrent(
	...
	async () => {
		...
		const stored = await miscRedis.get(redisKeyFor({ orgId, eventId }));
...
test.concurrent(
	...
	async () => {
		...
		const afterFailure = await miscRedis.get(redisKeyFor({ orgId, eventId }));
		...
		const afterSuccess = await miscRedis.get(redisKeyFor({ orgId, eventId }));
...
test.concurrent(
	...
	async () => {
		...
		await waitForCondition({
			condition: async () =>
				(await miscRedis.get(redisKeyFor({ orgId, eventId }))) === "processing",
...
test.concurrent(
	...
	async () => {
		...
		await waitForCondition({
			condition: async () =>
				(await miscRedis.get(redisKeyFor({ orgId, eventId }))) === "completed",
