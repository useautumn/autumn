...
| `invoice_credit` flag saves on a pooled feature (itemization is per plan item, not per feature) | ✓ t2 |
| `invoice_credit` flag saves with an included-only plan item | ✓ t3 |
