...
test.concurrent(
	`${chalk.yellowBright("catalogV2 plan-licenses: duplicate license_plan_id → 400")}`,
	async () => {
		const { autumnV2_3, ctx } = await initScenario({ setup: [], actions: [] });
		const parentId = uniqueTestId("cv2_lic_dup_p");
		const childId = uniqueTestId("cv2_lic_dup_c");
						...
						plans: [
							{
								plan_id: parentId,
								licenses: [
									{ license_plan_id: childId, included: 1 },
									{ license_plan_id: childId, included: 2 },
								],
							},
						],
					}),
