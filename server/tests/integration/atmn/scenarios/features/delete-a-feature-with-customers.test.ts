	...
	async () => {
		...
		const scenario = await initAtmnScenario({
			...
			config: configBody({
				...
				plans: `\n\t\tplan({ active: true, planId: "${planId}", name: "Delete With Customer Plan", items: [{ featureId: "${featureId}", included: 100 }] }),`,
			}),
		});
		...
		try {
			...
			scenario.writeConfig(
				atmnConfigSource({
					body: configBody({
						features: "",
						plans: `\n\t\tplan({ active: true, planId: "${planId}", name: "Delete With Customer Plan" }),`,
					}),
				}),
			);
