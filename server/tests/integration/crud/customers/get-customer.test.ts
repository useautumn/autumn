...
test.concurrent(
	`${chalk.yellowBright("get-customer: multi-entity customer returns correct balances across API versions")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("get-customer: expand empty array returns items")}`,
	async () => {
		const messagesItem = items.monthlyMessages({ includedUsage: 100 });
...
test.concurrent(
	`${chalk.yellowBright("get-customer: v2.1 returns boolean features in flags")}`,
	async () => {
		const dashboardItem = items.dashboard();
		const messagesItem = items.monthlyMessages({ includedUsage: 100 });
...
test.concurrent(
	`${chalk.yellowBright("get-customer: v2 returns boolean features in balances")}`,
	async () => {
		const dashboardItem = items.dashboard();
		const messagesItem = items.monthlyMessages({ includedUsage: 100 });
		const pro = products.pro({
...
test.concurrent(
	`${chalk.yellowBright("get-customer: created boolean balance is returned as flag with expires_at")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("get-customer: customer cache updates after entity attach")}`,
	async () => {
		const creditsItem = items.monthlyCredits({ includedUsage: 200 });
		const entityProd = products.base({
...
test.concurrent(
	`${chalk.yellowBright("get-customer: v2 balances.feature also expands flag features")}`,
	async () => {
		const dashboardItem = items.dashboard();
		const messagesItem = items.monthlyMessages({ includedUsage: 100 });
		const pro = products.pro({
