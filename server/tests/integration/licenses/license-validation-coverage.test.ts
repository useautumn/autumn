...
// TODO: Assert the capacity-conflict error `data.conflicts[]` payload shape
// ({ license_plan_id, requested_quantity, active_assignments, assignment_ids,
// entity_ids }). Infeasible via the test harness: the server serializes only
// { message, code } to the HTTP body, and the AutumnInt client drops everything
// but message/code — so the `data` payload never reaches the caller.
test.skip(
	`${chalk.yellowBright("licenses: capacity-conflict payload exposes conflicts[] shape")}`,
	() => {
