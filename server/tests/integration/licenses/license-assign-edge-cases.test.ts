 ...
 *  - licenses.unassign with an unknown assignment_id returns 200 with an
 ...
 *  - licenses.assign accepts pool_id to target an exact pool; without it the
 *    ambiguous case still fails with a clear error.
 */

import { expect, test } from "bun:test";
import { type LicensePoolResponse, ProductCatalogType } from "@autumn/shared";
...
test.concurrent(
	`${chalk.yellowBright("licenses-assign-edge: unassign with unknown assignment_id returns 404")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("licenses-assign-edge: pool_id disambiguates pools on subscription-less parents")}`,
	async () => {
