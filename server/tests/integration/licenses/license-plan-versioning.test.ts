 ...
 *    licenses.list_plan_licenses for the parent plan returns [].
 *
 * Green-success criteria (after fix):
 *  - Versioning a parent plan carries its plan license links forward;
 *    list_plan_licenses still returns the license with the same
 *    included_quantity and customize after the new version is created.
 */
...
type PlanLicenseRow = {
	...
	customize?: { items?: Array<{ included?: number }> } | null;
};
...
test.concurrent(
	...
	async () => {
		...
		expect(afterVersioning[0].customize?.items?.[0].included).toBe(100);
	},
);
