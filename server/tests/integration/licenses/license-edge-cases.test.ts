...
const setupAssignedLicense = async ({
	...
	await result.autumnV2_2.post("/licenses.set_plan_license", {
	...
	const assignResponse = (await result.autumnV2_2.post("/licenses.assign", {
...
test(
	...
	async () => {
		...
		const duplicate = (await autumnV2_2.post("/licenses.assign", {
			customer_id: customerId,
			entity_id: entities[0].id,
			plan_id: license.id,
		...
		expect(duplicate.assignment.metadata).toEqual({ source: "edge" });

		const assignments = (await autumnV2_2.post(
			"/licenses.list_assignments",
			{ customer_id: customerId, plan_id: license.id },
		)) as { list: unknown[] };
...
test(
	...
	async () => {
		...
		const first = (await autumnV2_2.post("/licenses.unassign", {
			assignment_id: assignment.id,
		})) as { assignment: { id: string; ended_at: number | null } };
		const second = (await autumnV2_2.post("/licenses.unassign", {
			assignment_id: assignment.id,
		})) as { assignment: { id: string; ended_at: number | null } };
...
test(
	...
	async () => {
		...
		const { list } = (await autumnV2_2.post("/licenses.list_plan_licenses", {
			parent_plan_id: parent.id,
...
test(
	...
	async () => {
		...
		const poolsBefore = (await autumnV2_2.post("/licenses.list_pools", {
			customer_id: customerId,
		})) as { list: LicensePoolResponse[] };
		expect(poolsBefore.list).toHaveLength(2);
		const poolA = poolsBefore.list.find(
			(pool) => pool.inventory.included_quantity === 1,
		);
		const poolB = poolsBefore.list.find(
			(pool) => pool.inventory.included_quantity === 2,
		);
		expect(poolA?.parent_subscription_id).toBeTruthy();
		expect(poolB?.parent_subscription_id).toBeTruthy();
		expect(poolA?.parent_subscription_id).not.toBe(
			poolB?.parent_subscription_id,
		);
		...
		await autumnV2_2.post("/licenses.assign", {
			...
			entity_id: entities[0].id,
			plan_id: license.id,
			parent_subscription_id: poolA?.parent_subscription_id,
		});
		await autumnV2_2.post("/licenses.assign", {
			customer_id: customerId,
			entity_id: entities[1].id,
			plan_id: license.id,
			parent_subscription_id: poolB?.parent_subscription_id,
		});
		...
		const poolsAfter = (await autumnV2_2.post("/licenses.list_pools", {
			customer_id: customerId,
		})) as { list: LicensePoolResponse[] };
		expect(
			poolsAfter.list.find(
				(pool) => pool.parent_subscription_id === poolA?.parent_subscription_id,
			)?.inventory,
		).toMatchObject({ included_quantity: 1, assigned: 1, available: 0 });
		expect(
			poolsAfter.list.find(
				(pool) => pool.parent_subscription_id === poolB?.parent_subscription_id,
			)?.inventory,
		).toMatchObject({ included_quantity: 2, assigned: 1, available: 1 });
	},
);
