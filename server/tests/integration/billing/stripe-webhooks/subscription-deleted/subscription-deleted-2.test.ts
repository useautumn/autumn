/**
 * Customer Subscription Deleted Webhook Tests (slice 2/2)
 *
 ...
 */
test.concurrent(
	`${chalk.yellowBright("sub.deleted: cancel after end_of_cycle via Stripe")}`,
	async () => {
 ...
 */
test.concurrent(
	`${chalk.yellowBright("sub.deleted: cancel subscription with add-on via Stripe")}`,
	async () => {
