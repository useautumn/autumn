...
// ═══════════════════════════════════════════════════════════════════════════════
// TEST 5: One-off + recurring is allowed in checkout
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Scenario:
 * - Customer with NO payment method
 * - Product has one-off item + recurring item
 *
 * Expected Result:
 * - Should work because one-off is excluded from interval check
 */
test.concurrent(`${chalk.yellowBright("checkout: one-off + recurring works")}`, async () => {
	const customerId = "stripe-checkout-oneoff-recurring";

	// Monthly prepaid messages
	const monthlyPrepaidItem = items.prepaidMessages({
		includedUsage: 0,
		billingUnits: 100,
		price: 10,
	...
	const mixedProduct = products.pro({
		id: "oneoff-recurring",
		items: [monthlyPrepaidItem, oneOffItem],
	});
	...
	// Should succeed - one-off doesn't count toward interval check
