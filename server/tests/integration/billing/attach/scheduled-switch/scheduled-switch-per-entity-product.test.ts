 ...
 */
test.concurrent(
	`${chalk.yellowBright("scheduled-switch-per-entity 1: premium to pro with per-entity consumable messages (usage resets)")}`,
	async () => {
 ...
 */
test.concurrent(
	`${chalk.yellowBright("scheduled-switch-per-entity 2: premium to free with per-entity consumable + FREE allocated (mixed behavior)")}`,
	async () => {
 ...
 */
test.concurrent(
	`${chalk.yellowBright("scheduled-switch-per-entity 3: downgrade with overage - billed at cycle end")}`,
	async () => {
