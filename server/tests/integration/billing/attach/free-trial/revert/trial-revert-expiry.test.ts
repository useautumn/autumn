...
// ═══════════════════════════════════════════════════════════════════════════════
// TEST 6: Cancel trial when previous plan was expired during trial
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Scenario:
 * - Customer on pro → enterprise revert trial
 * - During trial, the paused pro is expired (simulating Stripe subscription.deleted)
 * - User cancels the enterprise trial immediately
 *
 * Expected:
 * - Enterprise → Expired
 ...
 */
test(
	`${chalk.yellowBright("trial-revert-expiry 6: cancel trial does not restore expired previous plan")}`,
	async () => {
		const customerId = "trial-revert-cancel-expired-prev";
		...
		// Verify: enterprise expired, pro stays expired (NOT reactivated)
		const afterCancel = await CusService.getFull({
			ctx,
			idOrInternalId: customerId,
			inStatuses: ALL_STATUSES,
		...
		const expiredPro = afterCancel.customer_products.find(
			(cp) => cp.product_id === pro.id,
		);
...
// ═══════════════════════════════════════════════════════════════════════════════
// TEST 7: Convert revert to bill via update — paused plan expires at trial end
// ═══════════════════════════════════════════════════════════════════════════════

test(
	`${chalk.yellowBright("trial-revert-expiry 7: convert revert to bill — paused plan expires at trial end")}`,
	async () => {
		const customerId = "trial-revert-convert-bill";
		...
		await autumnV2.billing.attach<AttachParamsV1Input>(attachParams);

		// Convert from revert to bill via update
		await autumnV1.subscriptions.update(
			{
				customer_id: customerId,
				product_id: enterprise.id,
		...
		// Verify on_trial_end changed to bill, pro still paused
		...
		const updatedEnterprise = afterUpdate.customer_products.find(
			(cp) => cp.product_id === enterprise.id,
		);
		const stillPausedPro = afterUpdate.customer_products.find(
			(cp) => cp.product_id === pro.id,
		);

		expect(updatedEnterprise).toBeDefined();
		expect(updatedEnterprise!.on_trial_end).toBe("bill");
		expect(updatedEnterprise!.status).toBe(CusProductStatus.Active);
		expect(updatedEnterprise!.trial_ends_at).toBeDefined();
		expect(stillPausedPro).toBeDefined();
		expect(stillPausedPro!.status).toBe(CusProductStatus.Paused);

		// Expire the trial via cron
		const pastTrialEnd = Date.now() - 60_000;
		await db
			.update(customerProducts)
			.set({ trial_ends_at: pastTrialEnd })
			.where(eq(customerProducts.id, updatedEnterprise!.id));

		await runProductCron({ ctx: { db, logger } });

		// Verify: both expired (cron cleans up paused plan)
		const afterCron = await CusService.getFull({
			ctx,
			idOrInternalId: customerId,
			inStatuses: ALL_STATUSES,
		...
		const expiredProAfterCron = afterCron.customer_products.find(
			(cp) => cp.product_id === pro.id,
		);

		expect(expiredEnterprise!.status).toBe(CusProductStatus.Active);
		expect(expiredProAfterCron!.status).toBe(CusProductStatus.Expired);
	},
);
