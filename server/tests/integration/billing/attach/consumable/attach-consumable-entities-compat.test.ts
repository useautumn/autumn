...
// ═══════════════════════════════════════════════════════════════════════════════
// TEST 4: Mixed v1/v2 with different products on entities
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Scenario:
 * - Entity 1: v1 attach Pro ($20/month, 100 included, $1/10 units, billingUnits=10)
 * - Entity 2: v2 attach Premium ($50/month, 200 included, $2/25 units, billingUnits=25)
 ...
 * - After cycle:
 *   - Entity 1 (v1 Pro): $20 base + $8 overage = $28
 *   - Entity 2 (v2 Premium): $50 base + $8 overage = $58
 * - Subscription items are correct
 */
test.concurrent(`${chalk.yellowBright("attach consumable compat: v1 Pro entity + v2 Premium entity → both charged correctly")}`, async () => {
	const customerId = "attach-compat-v1pro-v2prem";
	...
	const { autumnV1, ctx, entities } = await initScenario({
		...
		actions: [
			// v1 attach Pro to entity 0
			s.attach({ productId: pro.id, entityIndex: 0 }),
			// v2 attach Premium to entity 1
			s.billing.attach({
	...
	// Calculate expected overages
	// Entity 1 (v1 Pro): ceil(75/10) = 8 billing units → $8
	const entity1Overage = calculateExpectedInvoiceAmount({
		items: pro.items,
	...
	// Entity 2 (v2 Premium): ceil(89/25) = 4 billing units → $8
	const entity2Overage = calculateExpectedInvoiceAmount({
		items: premium.items,
