...
// ═══════════════════════════════════════════════════════════════════════════════
// TEST 5: Customer + Entity with consumable - both charged correctly
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Scenario:
 * - Customer has Pro with consumable messages attached at customer level
 * - Entity 1 also has Pro with consumable messages attached
 * - Customer level: track 300 messages (200 overage → $20)
 * - Entity level: track 150 messages (50 overage → $5)
 * - Advance to next billing cycle
 *
 * Expected Result:
 * - Initial invoices: $20 (customer) + $20 (entity) = $40
 * - After cycle:
 *   - Customer invoice: $20 base + $20 overage = $40
 *   - Entity invoice: $20 base + $5 overage = $25
 * - Both balances should reset to 100
 */
test.concurrent(`${chalk.yellowBright("attach v2 consumable entity: customer + entity → both charged correctly")}`, async () => {
	const customerId = "attach-v2-cons-ent-cus-ent";
	...
	const { autumnV1, entities, ctx } = await initScenario({
		...
		actions: [
			// Attach pro at customer level (no entityIndex)
			s.attach({ productId: pro.id }),
			// Attach pro at entity level
			s.attach({ productId: pro.id, entityIndex: 0, timeout: 4000 }),
			// Track usage at customer level
			s.track({ featureId: TestFeature.Messages, value: 300 }), // 200 overage at customer level
			// Track usage at entity level
			s.track({ featureId: TestFeature.Messages, value: 150, entityIndex: 0 }), // 50 overage at entity level
			s.advanceToNextInvoice({ withPause: true }),
		],
	});
	...
	// Calculate expected overages
	// Customer: (300 - 100) * $0.10 = $20
	const customerOverage = calculateExpectedInvoiceAmount({
		items: pro.items,
		usage: [{ featureId: TestFeature.Messages, value: 300 }],
		options: { includeFixed: false, onlyArrear: true },
	});
	expect(customerOverage).toBe(20);

	// Entity: (150 - 100) * $0.10 = $5
	const entityOverage = calculateExpectedInvoiceAmount({
		items: pro.items,
		usage: [{ featureId: TestFeature.Messages, value: 150 }],
		options: { includeFixed: false, onlyArrear: true },
	});
	expect(entityOverage).toBe(5);
	...
	// Should have 4 invoices total:
	// - 2 initial invoices ($20 each for customer and entity)
	// - 1 renewal invoices ($20 + overage each)
	expectCustomerInvoiceCorrect({
		customer: customerAfterAdvance,
		count: 3,
	...
	// Verify customer-level balance (from customer-level product only)
	// Note: customer.features aggregates ALL balances (customer + entities)
	// Customer product balance should be 100 after reset
	// Entity product balance should be 100 after reset
	// Total customer balance = 100 + 100 = 200
	expect(customerAfterAdvance.features[TestFeature.Messages].balance).toBe(200);

	// Verify entity balance: 100 (own) + 100 (inherited from customer) = 200
	const entity = await autumnV1.entities.get(customerId, entities[0].id);
	expect(entity.features[TestFeature.Messages].balance).toBe(200);
});
