...
import { MigrationRunStatus, ms } from "@autumn/shared";
...
test(`${chalk.yellowBright("abandoned run: a dispatched run with no trigger handle settles on age alone")}`, async () => {
	const suffix = `${Date.now().toString(36)}c`;
	const customerId = `abandoned-nohandle-${suffix}`;
	const plan = products.base({ id: `nohandle-${suffix}`, items: [] });
	...
	const migration = await autumnV2_2.migrationsV2.deleteAndCreate({
		id: `nohandle-mig-${suffix}`,
		filter: { customer: { plan: { plan_id: plan.id } } },
		operations: {
			customer: [
				{ type: "update_plan", plan_filter: { plan_id: plan.id }, version: 2 },
			],
		},
		no_billing_changes: true,
	});

	// The handle is persisted in a second write after dispatch; losing it leaves
	// a run that can never be checked against the platform.
	const inserted = await migrationRunRepo.insert({
		ctx,
	...
	await migrationRunRepo.update({
		...
		updates: {
			status: MigrationRunStatus.Running,
			started_at: Date.now() - ms.hours(25),
		},
	});

	await reconcileAbandonedRuns({
		ctx,
		runs: [await statusOf({ ctx, internalId: inserted.internal_id })],
	});

	const settled = await statusOf({ ctx, internalId: inserted.internal_id });
	expect(settled.status).toBe(MigrationRunStatus.Failed);

	// A young handle-less run is still left alone.
	const young = await migrationRunRepo.insert({
		ctx,
		insert: { migration_internal_id: migration.internal_id, dry_run: false },
	});
	if (!young) throw new Error("Could not claim the young run");
	await migrationRunRepo.update({
		ctx,
		internalId: young.internal_id,
		updates: {
			status: MigrationRunStatus.Running,
			started_at: Date.now() - ms.minutes(30),
		},
	});
	await reconcileAbandonedRuns({
		ctx,
		runs: [await statusOf({ ctx, internalId: young.internal_id })],
	});
	expect((await statusOf({ ctx, internalId: young.internal_id })).status).toBe(
		MigrationRunStatus.Running,
	);
