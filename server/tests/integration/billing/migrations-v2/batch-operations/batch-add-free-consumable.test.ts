...
test.concurrent(`${chalk.yellowBright("batch metered add: pro plan anchors to each customer's billing cycle anchor")}`, async () => {
...
test.concurrent(`${chalk.yellowBright("batch metered add: free plan anchors to cusProduct.starts_at")}`, async () => {
...
test.concurrent(`${chalk.yellowBright("batch metered add: aligns to a same-interval sibling's anchor")}`, async () => {
