...
test.concurrent(`${chalk.yellowBright("migrations update_plan states: end-of-cycle cancel survives price update")}`, async () => {
...
test.concurrent(`${chalk.yellowBright("migrations update_plan states: entity scheduled and canceling states survive")}`, async () => {
	...
	await expectProductNotPresent({ customer: entity2Before, productId: pro.id });
	const scheduledIdsBefore = await getScheduledIds({
		ctx,
		customerId,
		productId: pro.id,
	...
	await expectProductNotPresent({ customer: entity2After, productId: pro.id });
	expect(
		await getCustomerProductPriceAmounts({
			ctx,
			customerId,
...
test.concurrent(`${chalk.yellowBright("migrations update_plan states: multi-product scheduled downgrade and canceling addon survive")}`, async () => {
	...
	await expectNoExpiredCustomerProducts({ ctx, customerId, productId: addon.id });
	await expectStripeSubscriptionCorrect({ ctx, customerId });
});
