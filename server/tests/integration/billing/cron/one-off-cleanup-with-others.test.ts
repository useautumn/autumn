...
const getFullCustomer = async (customerId: string): Promise<FullCustomer> => {
...
const countProductsByStatus = ({
	...
	return fullCus.customer_products.filter(
		(cp) => cp.product.id === productId && cp.status === status,
	).length;
};
...
test.concurrent(`${chalk.yellowBright("cleanup: oneoff-monthly-messages-both-active")}`, async () => {
	...
	const fullCus = await getFullCustomer(customerId);
	expect(countActiveProducts({ fullCus, productId: oneOff.id })).toBe(2);
});
...
test.concurrent(`${chalk.yellowBright("cleanup: lifetime-then-monthly-both-active")}`, async () => {
	...
	const fullCusAfter = await getFullCustomer(customerId);
	expect(
		countActiveProducts({
	...
	expect(
		countActiveProducts({ fullCus: fullCusAfter, productId: oneOffMonthly.id }),
	).toBe(1);
});
...
test.concurrent(`${chalk.yellowBright("cleanup: oneoff-prepaid-boolean-only-latest")}`, async () => {
	...
	const fullCus = await getFullCustomer(customerId);
	expect(countActiveProducts({ fullCus, productId: oneOff.id })).toBe(1);
	expect(
...
test.concurrent(`${chalk.yellowBright("cleanup: oneoff-boolean-newer-without-boolean")}`, async () => {
	...
	const fullCus = await getFullCustomer(customerId);
	expect(countActiveProducts({ fullCus, productId: oneOffWithBool.id })).toBe(
		1,
	);
	expect(countActiveProducts({ fullCus, productId: oneOffNoBool.id })).toBe(1);
});
