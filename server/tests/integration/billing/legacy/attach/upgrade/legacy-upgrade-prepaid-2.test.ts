/**
 * Legacy Upgrade Tests - Prepaid Billing (slice 2 of 2)
 *
...
test.concurrent(
	`${chalk.yellowBright("legacy-upgrade-prepaid 3: pro → premium with explicit options (quantity excludes allowance)")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("legacy-upgrade-prepaid 4: pro → premium without options (quantity inherits)")}`,
	async () => {
