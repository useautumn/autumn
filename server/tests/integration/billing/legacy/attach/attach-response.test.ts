/**
 * Legacy Attach Response Format Tests (slice 1 of 2)
 *
 * Migrated from:
 ...
 * across different attach scenarios (new, upgrade, one-off).
 */
...
test.concurrent(
	`${chalk.yellowBright("attach-response 1: new attach, no card → checkout_url")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("attach-response 2: upgrade response")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("attach-response 5: one-off attach, card on file")}`,
	async () => {
