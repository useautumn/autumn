...
test.concurrent(
	`${chalk.yellowBright("update-sub anchor-reset-errors 1: one-off product is blocked")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("update-sub anchor-reset-errors 2: cancel action with anchor reset is blocked")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("update-sub anchor-reset-errors 3: active trial is blocked")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("update-sub anchor-reset-errors 5: past scheduled anchor is blocked")}`,
	async () => {
		const customerId = "update-sub-anchor-err-past";
		const pro = products.pro({
			id: "pro",
			items: [items.monthlyMessages({ includedUsage: 100 })],
		});
		...
		await expectAutumnError({
			...
			func: async () => {
				await autumnV2_3.subscriptions.update<UpdateSubscriptionV1ParamsInput>({
					customer_id: customerId,
					plan_id: pro.id,
					billing_cycle_anchor: advancedTo - 24 * 60 * 60 * 1000,
				});
			},
		});
	},
