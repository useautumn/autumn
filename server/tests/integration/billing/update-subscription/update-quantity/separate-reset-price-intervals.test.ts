...
test.concurrent(
	`${chalk.yellowBright("split-interval prepaid: attach, update quantity, cron monthly reset")}`,
	async () => {
		...
		expect(state.customerEntitlement.entitlement.interval).toBe(
			EntInterval.Month,
		);
		...
		expect(
			stateAfterCronReset.customerEntitlement.next_reset_at,
		).toBeGreaterThan(Date.now());
	},
);

test.concurrent(
	`${chalk.yellowBright("split-interval prepaid: lazy customer get resets monthly entitlement")}`,
	async () => {
		const suffix = getSuffix();
		const customerId = `split-int-lazy-${suffix}`;
		const planId = `split_int_lazy_${suffix}`;
		...
		// The read above reset the row on the worker; Postgres lands that a moment later.
		const stateAfterLazyReset = await pollUntilAsserted({
			fetch: () => getSplitCustomerProductState({ ctx, customerId, planId }),
			assert: (state) => expect(state.customerEntitlement.balance).toBe(1000),
			timeoutMs: 15_000,
			intervalMs: 500,
		});
		expect(stateAfterLazyReset.customerEntitlement.balance).toBe(1000);
		expect(
			stateAfterLazyReset.customerEntitlement.next_reset_at,
		).toBeGreaterThan(Date.now());
		expect(stateAfterLazyReset.customerEntitlement.separate_interval).toBe(
			true,
		);
		expect(
...
test.concurrent(
	`${chalk.yellowBright("split-interval prepaid: quarterly invoice.created does not reset monthly entitlement")}`,
	async () => {
		const suffix = getSuffix();
		...
		expect(
			rawStateAfterQuarterlyInvoice.customerEntitlement.next_reset_at,
		).toBe(firstNextResetAt);
		...
		expect(
			rawStateAfterQuarterlyInvoice.customerEntitlement.reset_cycle_anchor,
		).toBe(rawStateAfterQuarterlyInvoice.customerProduct.billing_cycle_anchor);
