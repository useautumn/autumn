...
test.concurrent(
	`${chalk.yellowBright("per-entity: add per-entity feature to free product")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("per-entity: add consumable per-entity feature and track overage")}`,
	async () => {
		const messagesItem = items.monthlyMessages({ includedUsage: 100 });
		const free = products.base({ items: [messagesItem] });
...
test.concurrent(
	`${chalk.yellowBright("per-entity: increase included usage preserves entity usage")}`,
	async () => {
		...
		expectCustomerFeatureCorrect({
			...
			balance:
				updatedPerEntityMessages.included_usage * 2 -
				entity1Usage -
				entity2Usage,
...
test.concurrent(
	`${chalk.yellowBright("per-entity: change from consumable to prepaid billing")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("per-entity: upgrade free to pay-per-use per-entity")}`,
	async () => {
