 ...
 */
test.concurrent(
	`${chalk.yellowBright("uncancel + update quantity")}`,
	async () => {
		...
		// A tracked deduction reaches Postgres via Redis then the queue, so poll
		// until it lands instead of asserting the first read.
		...
		// Balance reflects the new quantity minus the preserved usage
		await expectCustomerFeatureCorrect({
			autumn: autumnV1,
			customerId,
			featureId: TestFeature.Messages,
			balance: newQuantity - messagesUsage,
			usage: messagesUsage,
		});
 ...
 */
test.concurrent(
	`${chalk.yellowBright("uncancel + custom plan (items)")}`,
	async () => {
		...
		expect(
			customerAfterUncancel.features?.[TestFeature.Messages]?.balance,
		).toBe(200);
 ...
 */
test.concurrent(
	`${chalk.yellowBright("uncancel trialing product")}`,
	async () => {
