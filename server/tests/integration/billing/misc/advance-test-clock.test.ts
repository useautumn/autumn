...
test("billing.advance_test_clock advances a customer's Stripe clock", async () => {
	...
	const response = AdvanceTestClockResponseSchema.parse(
		await autumnV2_2.post("/billing.advance_test_clock", {
	...
	await expect(
		autumnV2_2.post("/billing.advance_test_clock", {
			customer_id: customerId,
