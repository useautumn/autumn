...
import { expectCheckoutProductsSettled } from "@tests/integration/billing/multi-attach/utils/expectCheckoutProductsSettled";
import {
...
// ═══════════════════════════════════════════════════════════════════
test.concurrent(
	`${chalk.yellowBright("multi-attach checkout: one-off add-on + main recurring")}`,
	async () => {
		...
		// Verify both products attached (webhook-gated — poll, never blind-sleep)
		await expectCheckoutProductsSettled({
			org: ctx.org,
			env: ctx.env,
			autumn: autumnV1,
			customerId,
...
// ═══════════════════════════════════════════════════════════════════
test.concurrent(
	`${chalk.yellowBright("multi-attach checkout: two recurrings in different groups")}`,
	async () => {
...
// ═══════════════════════════════════════════════════════════════════
test.concurrent(
	`${chalk.yellowBright("multi-attach checkout: two plans with prepaid features")}`,
	async () => {
