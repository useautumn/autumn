...
describe(`${chalk.yellowBright("cancel-addon2: Attach pro + monthly add on, then cancel monthly add on immediately")}`, () => {
	const customerId = "cancel-addon2";
	...
	test("should attach pro and monthly add on, then cancel monthly add on immediately", async () => {
		...
		expect(customer.invoices?.[0]?.total).toBe(-20);
	});
});
