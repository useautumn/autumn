/** Customer spend-limit check tests (slice 2 of 2). */
...
test.concurrent(
	`${chalk.yellowBright("check-customer-spend-limit5: customer spend limit applies when customer inherits per-entity balances")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("check-customer-spend-limit6: disabled customer spend limit no longer caps direct customer checks")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("check-customer-spend-limit8: disabled customer spend limit no longer caps inherited per-entity checks across entities")}`,
	async () => {
		const perEntityProduct = products.base({
