...
test.concurrent(
	...
	async () => {
		...
		const { autumnV2_1, customerId } = await initScenario({
			customerId: "track-customer-credit-day-cap-1",
			setup: [
				s.customer({ paymentMethod: "success", testClock: false }),
				s.products({ list: [customerProduct] }),
			],
...
test.concurrent(
	...
	async () => {
		...
		const { autumnV2_1, customerId } = await initScenario({
			customerId: "track-customer-setusage-guard-1",
			setup: [
				s.customer({ paymentMethod: "success", testClock: false }),
				s.products({ list: [customerProduct] }),
			],
...
test.concurrent(
	...
	async () => {
		...
		const { autumnV2_1, customerId } = await initScenario({
			customerId: "track-customer-compound-cap-1",
			setup: [
				s.customer({ paymentMethod: "success", testClock: false }),
				s.products({ list: [customerProduct] }),
			],
...
test.concurrent(
	...
	async () => {
		...
		const { autumnV2_1, customerId } = await initScenario({
			customerId: "track-customer-concurrent-cap-1",
			setup: [
				s.customer({ paymentMethod: "success", testClock: false }),
				s.products({ list: [customerProduct] }),
			],
		...
		// The counter reflects only the winning track (5 action1 = 1 credit), not both.
		await expectCustomerFeatureCachedAndDb({
			autumn: autumnV2_1,
			customerId,
			featureId: TestFeature.Credits,
