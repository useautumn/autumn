...
test.concurrent(
	`${chalk.yellowBright("track-customer-spend-limit1: lifetime + consumable customer messages cap track overage")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("track-customer-spend-limit2: prepaid + consumable customer messages cap track overage")}`,
	async () => {
		const customerProduct = products.base({
...
test.concurrent(
	`${chalk.yellowBright("track-customer-spend-limit3: prepaid addon + consumable customer messages cap track overage")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("track-customer-spend-limit5: customer spend limit is enforced on aggregate per-entity balances")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("track-customer-spend-limit6: credit-system customer tracking uses converted credits and caps overage")}`,
	async () => {
...
test.concurrent(
	`${chalk.yellowBright("track-customer-spend-limit7: disabled customer spend limit no longer caps rejecting track")}`,
	async () => {
		const customerProduct = products.base({
...
test.concurrent(
	`${chalk.yellowBright("track-customer-spend-limit8: disabled customer spend limit no longer caps aggregate entity-product track across entities")}`,
	async () => {
