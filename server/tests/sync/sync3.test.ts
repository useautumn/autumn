...
describe(`${chalk.yellowBright(`sync/${testCase}: Testing sync track prepaid allocated feature with concurrent requests`)}`, () => {
	let db: DrizzleCli;
	...
	it("should only allow one concurrent seat allocation with 1 included seat and create no duplicate invoices", async () => {
        ...
        expect(successCount).to.equal(1, `Expected exactly 1 success (included seat), got ${successCount} | Results: ${results.map(r => r.status).join(", ")}`);
		expect(rejectedCount).to.equal(4, `Expected exactly 4 rejections (exceeded included), got ${rejectedCount} | Results: ${results.map(r => r.status).join(", ")}`);
		...
		expect(newInvoicesCreated).to.equal(0, `Expected 0 new invoices (only used included seat), got ${newInvoicesCreated}. Initial: ${initialInvoiceCount}, Final: ${finalInvoiceCount}`);
	});
});
