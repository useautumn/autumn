...
describe(`${chalk.yellowBright("cancel-addon4: Attach pro + usage add on, use overage, then attach failed payment method, and cancel usage add on immediately")}`, () => {
	const customerId = "cancel-addon4";
	...
	test("should attach pro and monthly add on, then attach failed payment method, and cancel monthly add on immediately", async () => {
		await autumn.attach({
