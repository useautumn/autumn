...
describe(`${chalk.yellowBright("subscription-update: proration configs - upgrades")}`, () => {
	...
	test("OnIncrease.BillImmediately - should charge full amount without proration", async () => {
		const product = createProductWithProrationConfig(
			"prorate_bill_immediately",
			OnIncrease.BillImmediately,
		);

		await initCustomerV3({
			ctx,
			customerId: `${customerId}-bill-immediately`,
			withTestClock: true,
			attachPm: "success",
		});
		...
		// Attach with 10 units
		await autumnV1.attach({
			customer_id: `${customerId}-bill-immediately`,
			product_id: product.id,
			options: [
				{
					feature_id: TestFeature.Messages,
					quantity: 10 * billingUnits, // 120 messages
				},
			],
		});

		const beforeInvoices = await autumnV1.customers.get<ApiCustomer>(
			`${customerId}-bill-immediately`,
		);
		const invoiceCountBefore = beforeInvoices.invoices?.length || 0;

		// Upgrade to 20 units (adding 10 units)
		...
		// Should charge FULL amount for 10 additional units (no proration)
		// 10 units × $8 = $80
		const fullAmount = 10 * pricePerUnit;
		expect(latestInvoice?.total).toBe(fullAmount);
	});
