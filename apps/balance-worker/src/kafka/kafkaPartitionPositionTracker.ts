...
export type KafkaPartitionProgress = {
...
export type KafkaPartitionPositionTrackerPort = {
	...
	observeHighWatermark(
		position: KafkaPartitionPosition & { highWatermark: bigint },
	): void;
	read(position: KafkaPartitionPosition): bigint | null;
	readProgress(position: KafkaPartitionPosition): KafkaPartitionProgress;
	waitUntil(
	...
	implements KafkaPartitionPositionTrackerPort
{
	...
	readProgress({
		topic,
		partition,
	}: KafkaPartitionPosition): KafkaPartitionProgress {
		validatePosition({ topic, partition, nextOffset: 0n });
		const key = positionKeyOf({ topic, partition });
		return {
			consumedNextOffset: this.nextOffsets.get(key) ?? null,
			highWatermark: this.highWatermarks.get(key) ?? null,
		};
	}
