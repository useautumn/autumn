...
import type { PartitionLogRange } from "../runtime/bootstrap/partitionBootstrapPlan.js";
...
export const createKafkaPartitionOutcomeFollower = ({
	...
	const readLogRange: PartitionOutcomeFollowerPort["readLogRange"] = async ({
		topic,
		partition,
		signal,
		...
		const logRange = {
			logStartOffset: parseKafkaRecordOffset({ offset: partitionRange.low }),
			logEndOffset: parseKafkaRecordOffset({ offset: partitionRange.high }),
		};
	...
	const startAndCatchUp: PartitionOutcomeFollowerPort["startAndCatchUp"] = ({
		...
		if (targetNextOffset < 0n) {
			return Promise.reject(
				new RangeError(`Invalid target next offset: ${targetNextOffset}`),
			);
		}
	...
	return {
		readLogRange,
		startAndCatchUp,
		readProgress: ({ topic, partition }) =>
			positionTracker.readProgress({ topic, partition }),
		stop,
		markUnavailable,
	};
};
