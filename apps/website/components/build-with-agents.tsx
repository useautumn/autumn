...
import { IconTick } from "@/app/constant";
...
const BrandMark = ({ title, d, ...props }: AgentMarkProps) => (
	<svg
		viewBox="0 0 24 24"
...
const IconCopy = ({ className, ...props }: SvgIconProps) => (
	<svg
		width="14"
		height="14"
		viewBox="0 0 14 14"
		...
		{...props}
	>
		<path d="M5 1H13V9H11V3H5V1Z" fill="currentColor" />
		<path d="M1 5H9V13H1V5ZM2.5 6.5V11.5H7.5V6.5H2.5Z" fill="currentColor" />
	</svg>
);

const AGENT_MARKS = [
	{ name: "Claude Code", d: CLAUDE_PATH },
	{ name: "Codex", d: OPENAI_PATH },
...
export default function AgentPromptPane({ active }: { active: boolean }) {
	const [typed, setTyped] = useState("");
	const [cursorVisible, setCursorVisible] = useState(true);
	const [copied, setCopied] = useState(false);
	const typingDone = typed.length >= PROMPT_TEXT.length;

	useEffect(() => {
		if (!active || typingDone) return;
		const timer = setTimeout(() => {
			setTyped(PROMPT_TEXT.slice(0, typed.length + 1));
		}, PROMPT_TYPING_SPEED_MS);
		return () => clearTimeout(timer);
	}, [active, typed, typingDone]);
	...
	return (
		<div className="border border-b-0 border-[#2A2A2A] bg-[#000000]/90">
			...
			<button
				...
				className="group flex w-full items-center gap-3 px-4 py-3 font-mono text-left cursor-pointer hover:bg-[#FFFFFF08] transition-colors duration-300"
			>
				<span
					aria-hidden="true"
				...
				<span className="flex-1 min-w-0 truncate text-[#BFBFBF] group-hover:text-white text-[13px] sm:text-sm tracking-[-2%] leading-none transition-colors duration-300">
					{typed}
					<span
						aria-hidden="true"
