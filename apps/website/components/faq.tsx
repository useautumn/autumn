					...
					{faqData.map((faq) => {
						...
						return (
							<div
								key={faq.id}
								className={cn(
									"group relative w-full cursor-pointer flex-col justify-center border-b border-[#292929] transition-colors duration-300 last:border-b-0",
							...
							>
								...
								<div className="relative z-10 px-4.5 md:px-8 lg:px-[20px] py-[30px]">
									...
									>
										<h3
											className={cn(
												"text-base lg:text-[18px] tracking-[-2%] transition-all duration-400",
												isOpen
										...
										<div className="overflow-hidden">
											<AnimatePresence initial={false}>
												{isOpen && (
													...
													>
														{faq.answer.split("\n\n").map((paragraph) => (
															<p key={`${faq.id}-${paragraph.slice(0, 16)}`}>
																{paragraph}
															</p>
														))}
													</motion.div>
