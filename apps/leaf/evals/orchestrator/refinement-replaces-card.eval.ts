...
export default defineEval({
	description:
		"A concrete refinement replaces the pending billing proposal and summarizes the changed terms and resulting payment.",
	async test(t) {
		const replacement = await t.send(
			"There is a pending proposal to attach pro_gen-attach-multi to customer gen-attach-multi at $1,035 per month. Replace it by adding a 14-day free trial; keep everything else unchanged.",
		);
