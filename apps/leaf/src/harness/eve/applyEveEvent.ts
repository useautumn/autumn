...
export type EveTurnOutcome =
	...
	| { kind: "suspended"; suspensions: Suspension[]; text: string }
	| { kind: "unreachable" };
...
/** One approval card payload per parked write: the write's own args plus the
 * option ids the resume path answers eve with, and the preview if this turn
 * took one. Every write records the rest of its batch, so a resume that only
 * decides part of the group still answers the requests it leaves behind. */
const suspensionsForGatedWrites = ({
	gated,
	progress,
}: {
	...
	gated.map((write) => {
		const options = approvalOptionIds({ options: write.options });
		return {
			...
			toolArgs: {
				...
				_eveSiblingRequestIds: gated
					.filter((sibling) => sibling.requestId !== write.requestId)
...
const parkOnInputRequest = async ({
	...
	if (parked?.kind === "gated") {
		return {
			kind: "suspended",
			suspensions: suspensionsForGatedWrites({
				gated: parked.gated,
				progress,
			}),
