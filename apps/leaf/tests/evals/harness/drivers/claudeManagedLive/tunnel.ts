...
// server's AUTUMN_PUBLIC_API_URL — the eval runs without touching dev-services. One domain =
...
export const openMockTunnel = async ({ mockPort }: { mockPort: number }) => {
	...
	try {
		...
		throw new Error(
			`ngrok tunnel for the eval mock MCP failed. Set NGROK_MOCK_MCP_URL to a dedicated reserved domain (separate from the dev server's AUTUMN_PUBLIC_API_URL), or free the domain currently in use. Original: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
