...
<Tip>
  Pass a `customer_id` to include customer-specific eligibility info like the attach action (activate, upgrade, downgrade), subscription status, and whether a free trial is available.
</Tip>
...
// - attachAction: 'activate', 'upgrade', 'downgrade', 'none', or 'purchase'
// - status: 'active' or 'scheduled' (if the customer has this plan)
// - canceling: whether the plan is set to cancel
// - trialing: whether the customer is on a free trial
// - trialAvailable: whether the customer can use the trial
```
...
<DynamicParamField body="customer_id" type="string">
  Customer ID to include eligibility info (attach action, subscription status, trial availability).
</DynamicParamField>
...
<DynamicResponseField name="list" type="object[]">
  <Expandable title="properties">
    ...
    <DynamicResponseField name="customer_eligibility" type="object">
      Customer-specific eligibility info. Only present when a `customer_id` is provided.
      <Expandable title="properties">
