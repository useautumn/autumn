...
<Note>
  Under heavy load, this endpoint may return a `503` with error code `service_unavailable` and a `Retry-After` header. The condition is transient — retry after the indicated delay.
</Note>
