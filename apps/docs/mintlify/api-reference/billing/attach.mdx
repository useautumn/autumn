...
<DynamicParamField body="redirect_mode" type="'always' | 'if_required' | 'never'">
  Controls when to return a checkout URL. 'always' returns a URL even if payment succeeds, 'if_required' (default) only when payment action is needed, 'never' disables redirects entirely.
</DynamicParamField>
...
<DynamicParamField body="carry_over_balances" type="object">
  Whether to carry over balances from the previous plan during a plan change.
  <Expandable title="properties">
    <DynamicParamField body="enabled" type="boolean" required>
...
<DynamicParamField body="carry_over_usages" type="object">
  Whether to carry over usages from the previous plan during a plan change.
  <Expandable title="properties">
    <DynamicParamField body="enabled" type="boolean" required>
