...
<Note>
  Stripe products and prices are lazily created in both sandbox and production — only on the first `attach` that uses them, not when you create or update a plan in Autumn. This means you can pre-map an Autumn plan to an existing Stripe product/price before the first attach, and Autumn will reuse it instead of creating a duplicate. Passing `create_in_stripe: true` when creating a plan opts into immediate creation.
</Note>
