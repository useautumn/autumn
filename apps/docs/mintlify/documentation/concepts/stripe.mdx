...
When you connect a Stripe **sandbox** account, Autumn automatically syncs all your existing plans into it as Stripe products and prices, so you can test the full flow immediately.

For **production** Stripe accounts, Autumn does not create products and prices at connect time. They're created lazily on the first `attach` that uses them, so you can pre-map plans to existing Stripe products before that first attach and Autumn will reuse them instead of creating duplicates. See [How products and prices map](#how-products-and-prices-map) for details.
...
<Note>
  **When Stripe products and prices are created**

  - **Sandbox**: Autumn creates Stripe products and prices eagerly — when a plan is created or updated, and when you connect a new Stripe sandbox account (Autumn syncs all existing plans over).
  - **Production**: Autumn creates Stripe products and prices lazily — only on the first `attach` that uses them. Nothing is created at connect time or when you push a plan.

  Because production is lazy, you can pre-map an Autumn plan to an existing Stripe product/price before the first attach, and Autumn will reuse it instead of creating a duplicate.
</Note>
