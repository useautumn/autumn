...
<Warning>
You must create customers before calling `check` or `track`. These endpoints do not auto-create customers — if the customer doesn't exist, they return a `customer_not_found` error.
</Warning>
