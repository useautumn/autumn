...
<Warning>
**API version `2026-04-01` and later:** Customers must be created explicitly before calling `/check` or `/track`. These endpoints no longer auto-create customers — if the customer doesn't exist, they return a `customer_not_found` error. Earlier API versions retain the previous auto-create behavior.
</Warning>
