...
If you're using an [AI credit system](/examples/monetary-credits), you can track token usage directly with `trackTokens`. This automatically converts token counts to a dollar cost using OpenRouter pricing and your configured markup, then deducts from the customer's credit balance.
...
<Tip>
  If your organization has only one AI credit system feature, you can omit the `featureId` parameter — it will be auto-detected.
</Tip>
