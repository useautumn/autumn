...
If you're using an [AI credit system](/examples/credits), you can track token usage directly with `trackTokens`. This automatically converts token counts to a dollar cost using OpenRouter pricing and your configured markup, then deducts from the customer's credit balance.
