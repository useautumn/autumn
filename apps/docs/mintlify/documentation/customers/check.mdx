...
<Warning>
The customer must exist before calling `check`. If the customer hasn't been created yet, this endpoint returns a `customer_not_found` error. Use [`customers.getOrCreate`](/documentation/customers/creating-customers) to create customers first.
</Warning>
