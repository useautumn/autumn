...
<Note>
**API version `2026-04-01` and later:** The customer must be [created](/documentation/customers/creating-customers) before calling `check`. If the customer doesn't exist, the endpoint returns a `customer_not_found` error instead of auto-creating one. Earlier API versions auto-create customers on first `check` call.
</Note>
