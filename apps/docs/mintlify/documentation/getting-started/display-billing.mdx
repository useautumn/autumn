...
- **`attachAction`** — The action that would occur if this plan were attached: `activate`, `upgrade`, `downgrade`, `none`, or `purchase`
- **`status`** — The customer's current status with this plan: `active` or `scheduled`
- **`canceling`** — Whether the customer's active instance of this plan is set to cancel
- **`trialing`** — Whether the customer is currently on a free trial of this plan
...
export default function PricingPage() {
  ...
  return plans?.map((plan) => (
    ...
    >
      {buttonText[plan.customerEligibility?.attachAction] ?? "Get started"}
    </button>
  ));
}
```
...
// Each plan includes customerEligibility with attachAction, status, and trialAvailable
...
# Each plan includes customer_eligibility with attach_action, status, and trial_available
