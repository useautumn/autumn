...
<Steps>
    ...
    Autumn uses Stripe's **Custom Payment Methods** system so that Stripe can generate invoices and accurately track billing cycles for Vercel Marketplace purchases. In the Stripe dashboard, go to **Settings → Payments → Custom payment methods**, then click **Create a custom payment method**.
