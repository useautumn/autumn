...
<Note>
  Requires Node.js 22+, ESM imports, and `autumn-js` and `ai` (v7) as peer dependencies. AI SDK 6 applications should stay on the previous gateway release.
</Note>
...
OpenAI, Anthropic, and Google provider names are normalized automatically, including dotted SDK namespaces such as `openai.responses`, `anthropic.messages`, and `google.generative-ai`. Vercel's `gateway` namespace maps to `vercel`. Dots and slashes in the model ID are preserved. Other provider names pass through unchanged.

For a custom mapping, use `providerId`. Explicit overrides are used verbatim:
...
| `model` | `LanguageModelV4` | Yes | The AI SDK 7 language model to wrap |
| `customerId` | `string` | Yes | The Autumn customer ID to attribute usage to |
| `providerId` | `string` | No | Explicit pricing provider, used verbatim instead of the normalized model provider |
