...
When a plan bills a credit system **pay-per-use at a flat rate per credit** (for example `$0.10` per credit, or `$5` per 100 credits), Autumn treats the balance as invoice credits: every tracked usage is attributed to the feature that spent it, and the invoice lists one line per feature priced at that rate ("Premium messages, 40 units … $0.80") plus a "Credits applied" line refunding the credits the plan included. Balances like this can only be moved by tracked usage and cycle resets, so the invoice always matches the ledger.

Any other price shape (graduated tiers, prepaid packs, included-only or pooled items) bills as an ordinary overage. The decision is made per customer when the plan is attached, so changing a plan's price later never rewrites an existing customer's invoices.
