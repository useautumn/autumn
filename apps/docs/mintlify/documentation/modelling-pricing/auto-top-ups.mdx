...
Subscribe to [`billing.auto_topup_failed`](/api-reference/webhooks/billingAutoTopupFailed) to monitor auto top-ups that are skipped, blocked, declined, or fail before granting balance. The payload includes a machine-readable `reason`, a `retryable` flag, and any available invoice or provider error details.