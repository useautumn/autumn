...
<Update label="May 20th 2026">
  ...
  A new **`billing.updated`** webhook fires whenever a customer's plans change — covering plans that are activated, scheduled, updated in place, or expired. Each event includes the full `plan_changes` array with the subscription or purchase state after the change (status, period, trial, cancellation, and expiry timestamps).

  Use it to keep your own systems in sync with plan transitions without polling. See the [`billing.updated` reference](/api-reference/webhooks/billingUpdated).

  ## `entities.list` endpoint

  You can now list every entity attached to a customer via the new `entities.list` endpoint. Filter by customer, feature, or processor, and paginate with cursors to walk large entity sets.

  See the [list entities reference](/api-reference/entities/listEntities).

  ## Cursor-based pagination (API v2.3)

  Customers, entities, and events endpoints now support cursor-based pagination via a `start_cursor` query parameter and a `next_cursor` field in responses. Cursors are stable across data changes and significantly faster on large datasets than offset pagination.

  ## Trial behavior with `on_end`

  `billing.attach` trials now accept an **`on_end`** parameter on the `freeTrial` config:

  - `bill` (default): charge the customer when the trial ends.
  - `revert`: expire the trial and restore the customer's previous plan.

  Useful for offering risk-free upgrades that automatically roll back if the customer doesn't actively continue. See the [attach reference](/api-reference/billing/attach).

  ## Per-balance `deductions` on `/track`

  The `/track` response now includes a `deductions` array showing exactly which balances were consumed by each event, including the affected `balance_id`, `feature_id`, `plan_id`, amount deducted, and whether the deduction reset a balance.

  Lets you build per-feature usage views or audit trails directly from the track response. See the [track endpoint](/api-reference/core/track).

  ## Passkey sign-in

  You can now sign into the Autumn dashboard with a passkey. Register and manage passkeys from your account settings, and use them for faster, phishing-resistant logins (including autofill on supported browsers).

  ## Manual auto top-ups

  Auto top-ups can now be attached to a customer manually from the dashboard or via the API — useful for granting initial credit blocks or recovering from failed auto-charges without waiting for the next threshold event.

  ## Vercel resource pre-provisioning

  Vercel Marketplace integrations now provision a customer product as soon as a resource is created (with pre-authorization), so customers get immediate access instead of waiting for the first paid invoice.

  <AccordionGroup>
    <Accordion title="Improvements">
      - `billing.updated` webhook for plan lifecycle changes - [#1637](https://github.com/useautumn/autumn/pull/1637)
      - `entities.list` endpoint with filtering and pagination - [#1505](https://github.com/useautumn/autumn/pull/1505), [#1513](https://github.com/useautumn/autumn/pull/1513)
      - Cursor-based pagination and API v2.3 across customers, entities, and events - [#1596](https://github.com/useautumn/autumn/pull/1596)
      - `on_end` (`bill` / `revert`) trial behavior on `billing.attach` - [#1554](https://github.com/useautumn/autumn/pull/1554), [#1581](https://github.com/useautumn/autumn/pull/1581), [#1589](https://github.com/useautumn/autumn/pull/1589)
      - `deductions` array on `/track` responses with per-balance breakdown - [#1535](https://github.com/useautumn/autumn/pull/1535), [#1538](https://github.com/useautumn/autumn/pull/1538)
      - Passkey (WebAuthn) sign-in and management - [#1587](https://github.com/useautumn/autumn/pull/1587)
      - Manual auto top-up attach - [#1532](https://github.com/useautumn/autumn/pull/1532)
      - Live normalisation of expired auto top-up purchase limit windows - [#1520](https://github.com/useautumn/autumn/pull/1520)
      - Vercel resource pre-provisioning on create - [#1578](https://github.com/useautumn/autumn/pull/1578)
      - Cancel flow now handles plans with a future `starts_at` - [#1507](https://github.com/useautumn/autumn/pull/1507)
      - Stripe resources are reused across plan versions to keep IDs stable - [#1524](https://github.com/useautumn/autumn/pull/1524)
      - External payment processor signals on customer responses - [#1490](https://github.com/useautumn/autumn/pull/1490), [#1498](https://github.com/useautumn/autumn/pull/1498)
      - Favourite impersonation shortcuts in the command bar - [#1597](https://github.com/useautumn/autumn/pull/1597)
      - Refreshed dashboard sign-in page and settings - [#1555](https://github.com/useautumn/autumn/pull/1555)
      - Analytics chart and table polish, including `$plan_id` grouping - [#1568](https://github.com/useautumn/autumn/pull/1568), [#1599](https://github.com/useautumn/autumn/pull/1599), [#1613](https://github.com/useautumn/autumn/pull/1613)
      - Updated docs logo, favicon, and theme contrast - [#1645](https://github.com/useautumn/autumn/pull/1645), [#1646](https://github.com/useautumn/autumn/pull/1646)
    </Accordion>
    <Accordion title="Bug fixes">
      - Fixed tax rate ID handling on attach - [#1641](https://github.com/useautumn/autumn/pull/1641)
      - Fixed revert trial edge case - [#1635](https://github.com/useautumn/autumn/pull/1635)
      - Fixed `events.list` hook response typing - [#1633](https://github.com/useautumn/autumn/pull/1633)
      - Fixed ghost items appearing when deleting custom plan items - [#1628](https://github.com/useautumn/autumn/pull/1628), [#1629](https://github.com/useautumn/autumn/pull/1629)
      - Fixed dashboard customer cache invalidation - [#1612](https://github.com/useautumn/autumn/pull/1612)
      - Fixed webhook delivery race condition - [#1620](https://github.com/useautumn/autumn/pull/1620)
      - Fixed analytics tooltip and plan selection grouping - [#1608](https://github.com/useautumn/autumn/pull/1608), [#1624](https://github.com/useautumn/autumn/pull/1624)
      - Fixed sync of entity IDs and item logs - [#1607](https://github.com/useautumn/autumn/pull/1607), [#1614](https://github.com/useautumn/autumn/pull/1614)
      - Fixed several Vercel Marketplace billing edge cases - [#1558](https://github.com/useautumn/autumn/pull/1558), [#1579](https://github.com/useautumn/autumn/pull/1579)
      - Fixed schedule customize free price flow - [#1556](https://github.com/useautumn/autumn/pull/1556)
      - Fixed subscription detail item duplication - [#1574](https://github.com/useautumn/autumn/pull/1574), [#1575](https://github.com/useautumn/autumn/pull/1575)
      - Fixed attach when there are no billing changes - [#1586](https://github.com/useautumn/autumn/pull/1586)
      - Fixed checkout session race condition - [#1548](https://github.com/useautumn/autumn/pull/1548)
      - Fixed invoice refund select z-index in dialogs - [#1601](https://github.com/useautumn/autumn/pull/1601), [#1602](https://github.com/useautumn/autumn/pull/1602)
      - Fixed duplicate Stripe customer ID handling - [#1528](https://github.com/useautumn/autumn/pull/1528)
      - Fixed cache invalidation edge cases - [#1531](https://github.com/useautumn/autumn/pull/1531)
    </Accordion>
  </AccordionGroup>
</Update>
