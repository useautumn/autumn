...
<Update label="May 20th 2026">
  ...
  A new **`billing.updated`** webhook fires whenever a customer's plans are activated, scheduled, updated, or expired. The payload includes the full subscription, any scheduled change, and entity scope — so you can drive provisioning, notifications, or downstream syncs from a single event instead of stitching together attach, cancel, and renewal hooks.

  See the [billing.updated webhook reference](/api-reference/webhooks/billingUpdated).

  ## Cursor-based pagination on list endpoints

  List endpoints now support cursor-based pagination via a new `start_cursor` parameter and `next_cursor` response field. Pass an empty string for the first page, then carry `next_cursor` forward. Available on [`events.list`](/api-reference/events/listEvents), [`customers.list`](/api-reference/customers/listCustomers), and [`entities.list`](/api-reference/entities/listEntities) as part of API **v2.3**.

  ## Trial `on_end` behavior

  Free trials now accept an **`on_end`** option on `billing.attach` and subscription updates:

  - **`bill`** (default) — charges the customer when the trial completes.
  - **`revert`** — expires the trial and restores the customer's previous plan, so you can offer no-card or evaluation trials that automatically fall back instead of converting.

  See the [attach API reference](/api-reference/billing/attach).

  ## Passkey sign-in

  You can now sign in to the Autumn dashboard with a **passkey** (WebAuthn). Add and manage passkeys from your account settings — sign-in autofill is supported on browsers and devices that have one registered.

  ## Vercel resource pre-provisioning

  When a customer is created through the Vercel Marketplace, Autumn now provisions their plan immediately on resource creation using a payment preauthorization. Customers get access on first invoice instead of waiting for the first successful charge. See the [Vercel Marketplace guide](/documentation/external-providers/vercel-marketplace).

  ## `plan_id` in analytics

  Customer analytics now group and filter by **`plan_id`**, so you can see usage broken out per plan directly in the customer view. The chart legend, group-by selector, and row details have all been refreshed to support it.

  <AccordionGroup>
    <Accordion title="Improvements">
      - `billing.updated` webhook covering activation, scheduling, updates, and expiry across attach, multi-attach, migrate, restore, sync, and subscription-update flows - [#1637](https://github.com/useautumn/autumn/pull/1637)
      - Cursor-based pagination (`start_cursor` / `next_cursor`) on list endpoints - [#1596](https://github.com/useautumn/autumn/pull/1596)
      - `on_end: 'bill' | 'revert'` on trial configuration in `billing.attach` and subscription updates - [#1554](https://github.com/useautumn/autumn/pull/1554), [#1581](https://github.com/useautumn/autumn/pull/1581)
      - Passkey (WebAuthn) sign-in and management for the dashboard - [#1587](https://github.com/useautumn/autumn/pull/1587)
      - Vercel Marketplace customers pre-provisioned on resource creation with payment preauthorization - [#1578](https://github.com/useautumn/autumn/pull/1578)
      - `plan_id` available for grouping and filtering in customer analytics - [#1568](https://github.com/useautumn/autumn/pull/1568)
      - Analytics chart and table polish — refreshed legend, group-by selector, and row details - [#1613](https://github.com/useautumn/autumn/pull/1613)
      - Tags propagated to Svix webhook deliveries so you can filter by environment and event type - [#1617](https://github.com/useautumn/autumn/pull/1617)
      - Favourite orgs and users in the admin command bar for faster impersonation - [#1597](https://github.com/useautumn/autumn/pull/1597)
      - Dashboard sign-in page and branding refresh, updated logo and favicon
      - Migration UI cleanup and consistency pass across the dashboard
      - Larger database connection pool with new pool observability for faster dashboard reads - [#1625](https://github.com/useautumn/autumn/pull/1625), [#1643](https://github.com/useautumn/autumn/pull/1643)
    </Accordion>
    <Accordion title="Bug fixes">
      - `attach` now applies the correct Stripe tax rate ID - [#1641](https://github.com/useautumn/autumn/pull/1641)
      - Reverting a trial no longer leaves the customer in an inconsistent state - [#1635](https://github.com/useautumn/autumn/pull/1635)
      - `events.list` response typing fixed for the SDK - [#1633](https://github.com/useautumn/autumn/pull/1633)
      - Shadow feature flag evaluation corrected - [#1632](https://github.com/useautumn/autumn/pull/1632)
      - Deleting a custom item from a plan no longer leaves orphaned references - [#1629](https://github.com/useautumn/autumn/pull/1629)
      - No-op plan patches and ghost items on delete fixed with stable feature-based item keys - [#1628](https://github.com/useautumn/autumn/pull/1628)
      - Webhook delivery race condition resolved - [#1620](https://github.com/useautumn/autumn/pull/1620)
      - Customer-to-entity sync edge cases fixed; tolerance added to `hasCustomerProductEnded` - [#1614](https://github.com/useautumn/autumn/pull/1614), [#1607](https://github.com/useautumn/autumn/pull/1607)
      - Dashboard customer cache invalidation fixed - [#1612](https://github.com/useautumn/autumn/pull/1612)
      - Analytics `plan_id` parsing now preserves valid deductions when a single row fails - [#1608](https://github.com/useautumn/autumn/pull/1608)
      - Vercel customer filtering and provisioning edge cases - [#1579](https://github.com/useautumn/autumn/pull/1579)
      - `billing.attach` calls with no billing changes are now correctly treated as no-ops - [#1586](https://github.com/useautumn/autumn/pull/1586)
      - Subscription detail no longer shows duplicate line items - [#1575](https://github.com/useautumn/autumn/pull/1575)
      - Invoice refund select z-index fixed inside dialogs
    </Accordion>
  </AccordionGroup>
</Update>
