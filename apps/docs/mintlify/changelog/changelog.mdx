...
<Update label="May 20th 2026">
  ## API v2.3 with cursor-based pagination

  List endpoints now support cursor-based pagination. The new API version 2.3 ships with a `start_cursor` parameter on list endpoints (customers, entities, events) and updated SDKs that handle paging for you. Cursors are stable across large result sets and significantly faster than offset-based pagination.

  ## List entities endpoint

  A new **List entities** endpoint lets you fetch every entity attached to a customer in one call, with filtering and pagination built in. Pair it with the existing entity create / get / update / delete endpoints for full programmatic management of seats, workspaces, or any other entity model.

  See the [list entities reference](/api-reference/entities/listEntities).

  ## Trial `on_end` behaviour on `attach`

  You can now control what happens when a trial ends by passing `trial.on_end` to `billing.attach`. Choose whether the customer continues onto the paid plan, has the product expire, or cancels — useful for card-not-required trials, sales-led pilots, and time-boxed access.

  See the [trials guide](/documentation/modelling-pricing/trials) and [attach reference](/api-reference/billing/attach).

  ## Manual auto top-ups

  You can now trigger an auto top-up manually from the dashboard or API, in addition to the automatic threshold-based behaviour. Useful for letting customers explicitly buy more credits, or for handing off a top-up flow to a CSM.

  See the [auto top-ups guide](/documentation/modelling-pricing/auto-top-ups).

  ## Vercel Marketplace pre-provisioning

  Vercel Marketplace customers are now provisioned with their product the moment the resource is created — including a preauthorisation on the payment method — so installs land in a ready-to-use state without a follow-up attach.

  See the [Vercel Marketplace guide](/documentation/external-providers/vercel-marketplace).

  ## Passkey sign-in

  Sign in to the Autumn dashboard with a passkey. Add and manage passkeys from your account settings, and use browser autofill for one-tap sign-in across devices.

  ## Customer API logs in the dashboard

  Each customer now has a Stripe-style **API logs** panel showing every request made against their account — method, endpoint, status, latency, request/response bodies, and timestamps. Useful for debugging integrations without leaving the customer view.

  ## Plan-level analytics breakdowns

  Analytics charts and tables now group usage and revenue by `plan_id`, so you can see which plans are driving consumption and spend at a glance. The chart tooltip and table have also been polished for easier scanning.

  ## Refreshed dashboard UI

  We've rebuilt the dashboard on Base UI for snappier interactions, more consistent menus and dialogs, and a cleaned-up settings and sign-in experience. The migrations flow also got a major UX pass with clearer pickers, live status, and one-click "copy from previous phase".

  <AccordionGroup>
    <Accordion title="Improvements">
      - API v2.3 with `start_cursor` pagination on list endpoints and updated SDKs - [#1596](https://github.com/useautumn/autumn/pull/1596), [#1615](https://github.com/useautumn/autumn/pull/1615)
      - List entities endpoint - [#1505](https://github.com/useautumn/autumn/pull/1505), [#1513](https://github.com/useautumn/autumn/pull/1513)
      - `trial.on_end` on `billing.attach` (continue / expire / cancel) - [#1554](https://github.com/useautumn/autumn/pull/1554), [#1581](https://github.com/useautumn/autumn/pull/1581)
      - Manual auto top-up trigger from dashboard and API - [#1532](https://github.com/useautumn/autumn/pull/1532)
      - Live normalisation of expired auto top-up purchase-limit windows - [#1520](https://github.com/useautumn/autumn/pull/1520)
      - Vercel Marketplace pre-provisioning with preauthorisation on resource create - [#1578](https://github.com/useautumn/autumn/pull/1578)
      - Passkey (WebAuthn) sign-in and management UI - [#1587](https://github.com/useautumn/autumn/pull/1587)
      - Customer-level API logs / workbench panel - [#1525](https://github.com/useautumn/autumn/pull/1525)
      - `plan_id` grouping across analytics charts and tables - [#1568](https://github.com/useautumn/autumn/pull/1568), [#1613](https://github.com/useautumn/autumn/pull/1613)
      - Reusable Stripe resources across plan versions - [#1524](https://github.com/useautumn/autumn/pull/1524)
      - Migrations UI overhaul with clearer pickers and live status - [#1545](https://github.com/useautumn/autumn/pull/1545), [#1605](https://github.com/useautumn/autumn/pull/1605)
      - "Copy from previous phase" shortcut in the schedule product selector - [#1556](https://github.com/useautumn/autumn/pull/1556)
      - Cancel flow now handles subscriptions with a future `starts_at` - [#1507](https://github.com/useautumn/autumn/pull/1507)
      - Favourite orgs and users in the impersonation command bar - [#1597](https://github.com/useautumn/autumn/pull/1597)
      - Dashboard rebuilt on Base UI for faster, more consistent interactions - [#1519](https://github.com/useautumn/autumn/pull/1519), [#1542](https://github.com/useautumn/autumn/pull/1542)
      - Refreshed sign-in page and settings layout - [#1555](https://github.com/useautumn/autumn/pull/1555)
    </Accordion>
    <Accordion title="Bug fixes">
      - Honour configured tax rate ID on `billing.attach` - [#1641](https://github.com/useautumn/autumn/pull/1641)
      - Resolve webhook race condition that could drop events - [#1620](https://github.com/useautumn/autumn/pull/1620)
      - Disable Stripe Tax for Vercel customers and make metadata optional - [#1558](https://github.com/useautumn/autumn/pull/1558)
      - Treat Stripe schedules with no future phases as non-existent - [#1556](https://github.com/useautumn/autumn/pull/1556)
      - Prevent catalog base-price backfill on free custom schedule plans - [#1564](https://github.com/useautumn/autumn/pull/1564)
      - Stop one-off items from duplicating in the subscription detail sheet - [#1574](https://github.com/useautumn/autumn/pull/1574), [#1575](https://github.com/useautumn/autumn/pull/1575)
      - `attach` with no billing changes no longer creates a no-op Stripe update - [#1586](https://github.com/useautumn/autumn/pull/1586)
      - Fix delete behaviour for custom items so they no longer reappear - [#1629](https://github.com/useautumn/autumn/pull/1629)
      - Fix revert-trial issue when changing plans mid-trial - [#1635](https://github.com/useautumn/autumn/pull/1635)
      - `useListEvents` hook returns the correct response type - [#1633](https://github.com/useautumn/autumn/pull/1633)
      - Lock receipts now saved for unlimited-balance features - [#1534](https://github.com/useautumn/autumn/pull/1534)
      - Checkout session race condition resolved - [#1548](https://github.com/useautumn/autumn/pull/1548)
      - Dashboard customer cache invalidation fixes - [#1612](https://github.com/useautumn/autumn/pull/1612), [#1529](https://github.com/useautumn/autumn/pull/1529), [#1531](https://github.com/useautumn/autumn/pull/1531)
      - Sync edge cases for entities and item logs - [#1607](https://github.com/useautumn/autumn/pull/1607), [#1614](https://github.com/useautumn/autumn/pull/1614)
      - Various analytics tooltip and deductions fixes - [#1608](https://github.com/useautumn/autumn/pull/1608), [#1624](https://github.com/useautumn/autumn/pull/1624)
    </Accordion>
  </AccordionGroup>
</Update>
