...
<Update label="May 20th 2026">
  ## Trial `on_end` behavior

  Trials now accept an `on_end` parameter on `billing.attach` that controls what happens when the trial expires:

  - **`bill`** (default) — charge the customer and continue the subscription.
  - **`revert`** — expire the trial and restore the customer's previous plan, with no charge.

  Use `revert` to offer pay-up-front trials of a higher tier without forcing a downgrade flow at the end. The parameter is also surfaced on `billing.preview_attach`, `billing.update`, `billing.multi_attach`, `check`, and the `trial` object on customer responses.

  See the [attach API reference](/api-reference/billing/attach) and the [trials guide](/documentation/modelling-pricing/trials).

  ## `billing.updated` webhook

  A new **`billing.updated`** webhook fires whenever a customer's plans change — activations, scheduled changes, in-place updates, and expirations all flow through a single event. The payload includes `customer_id`, optional `entity_id`, and a `plan_changes` array describing every affected subscription, with the post-change subscription state attached.

  Use it as a single source of truth for syncing plan state to your own system without polling.

  See the [Plans Updated webhook reference](/api-reference/webhooks/billingUpdated).

  ## API v2.3: cursor-based pagination

  List endpoints now use cursor-based pagination on API v2.3. Pass `start_cursor` (omit for the first page) and read `next_cursor` from the response — `null` means you've reached the end. This replaces `offset`/`total`/`has_more` for stable pagination over large, frequently-changing datasets like events.

  Existing integrations on prior API versions continue to work unchanged.

  See the [List Events reference](/api-reference/events/listEvents).

  ## Passkey sign-in for the dashboard

  You can now register **passkeys** (WebAuthn) on your Autumn dashboard account. Manage passkeys from the Security card in **Settings → Account**, and sign in by selecting a saved passkey directly from the email field's autocomplete on the sign-in page — no password or magic link required.

  ## Vercel: pre-provision on resource create

  Vercel Marketplace installations now provision the customer product immediately when a resource is created, with payment preauthorization. Your customers get access to entitlements as soon as they install, rather than waiting on the first invoice cycle.

  ## Analytics improvements

  - **Plan ID grouping** — group usage analytics by `plan_id` to compare consumption across plans on a single chart.
  - **Unlimited deductions** — analytics now correctly attribute usage to customers on unlimited entitlements, so unlimited users no longer disappear from charts.

  <AccordionGroup>
    <Accordion title="Improvements">
      - `on_end` parameter on trials with `bill` and `revert` modes — [#1554](https://github.com/useautumn/autumn/pull/1554), [#1581](https://github.com/useautumn/autumn/pull/1581)
      - New `billing.updated` webhook covering activations, scheduled changes, updates, and expirations — [#1637](https://github.com/useautumn/autumn/pull/1637)
      - API v2.3 with cursor-based pagination on list endpoints — [#1596](https://github.com/useautumn/autumn/pull/1596), [#1615](https://github.com/useautumn/autumn/pull/1615)
      - Passkey (WebAuthn) registration and sign-in on the dashboard — [#1587](https://github.com/useautumn/autumn/pull/1587)
      - Vercel Marketplace: provision customer product on resource create with payment preauthorization — [#1578](https://github.com/useautumn/autumn/pull/1578)
      - Analytics: group by `plan_id` and correct attribution for unlimited deductions — [#1608](https://github.com/useautumn/autumn/pull/1608)
      - Analytics chart and table polish, with refined plan selection and tooltip behavior — [#1599](https://github.com/useautumn/autumn/pull/1599), [#1613](https://github.com/useautumn/autumn/pull/1613)
      - Webhook payloads now carry `tags` for easier routing — [#1617](https://github.com/useautumn/autumn/pull/1617)
      - Dashboard frontend cleanup and refinements — [#1619](https://github.com/useautumn/autumn/pull/1619)
    </Accordion>
    <Accordion title="Bug fixes">
      - Tax rate ID is now applied correctly on `billing.attach` — [#1641](https://github.com/useautumn/autumn/pull/1641)
      - Reverting a trial after a plan change no longer leaves the customer in an inconsistent state — [#1635](https://github.com/useautumn/autumn/pull/1635)
      - Fixed a race condition in webhook delivery that could cause duplicate or missed events — [#1620](https://github.com/useautumn/autumn/pull/1620)
      - Plan edits with no changes are now a true no-op and won't trigger downstream sync — [#1628](https://github.com/useautumn/autumn/pull/1628)
      - Deleting custom plan items no longer leaves behind "ghost" entries on save — [#1629](https://github.com/useautumn/autumn/pull/1629)
      - Sync entities tolerates customer products that have just ended — [#1614](https://github.com/useautumn/autumn/pull/1614)
      - Reward redemption and coupon updates handled atomically — [#1600](https://github.com/useautumn/autumn/pull/1600)
      ...
      - Recharts tooltip replaced with a custom overlay so `plan_id` grouping renders reliably — [#1624](https://github.com/useautumn/autumn/pull/1624)
      - Dashboard customer cache invalidation fixed — [#1612](https://github.com/useautumn/autumn/pull/1612)
      - List Events response typing corrected — [#1633](https://github.com/useautumn/autumn/pull/1633)
      - Shadow feature flags applied consistently across the dashboard — [#1632](https://github.com/useautumn/autumn/pull/1632)
      - Various Vercel Marketplace edge cases fixed — [#1579](https://github.com/useautumn/autumn/pull/1579)
    </Accordion>
  </AccordionGroup>
</Update>
